#!/usr/bin/env python3
"""Write grading-detail.tsx in parts to avoid truncation."""
import os

target = "resources/js/pages/dosen/grading-detail.tsx"

# Part 1: Imports, interfaces, constants, helpers, Modal component
p1 = '''import DosenLayout from '@/layouts/dosen-layout';
...
interface AttRec {
    id: number; meeting_number: number; session_title: string; session_date: string;
    session_time: string; status: 'present'|'late'|'permit'|'sick'|'absent'|'rejected';
...
const tlDot = (s: string) => ({present:'bg-emerald-500',late:'bg-amber-500',permit:'bg-blue-500',sick:'bg-cyan-500',absent:'bg-red-500',rejected:'bg-rose-500'}[s] || 'bg-neutral-500');
const initials = (n: string) => n.split(' ').map((w: string) => w[0]).join('').substring(0,2).toUpperCase();
        ...
        <AnimatePresence>
            {show && (
                <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
                    <motion.div initial={{scale:0.9,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.9,opacity:0}} className="bg-white dark:bg-neutral-900 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-neutral-200 dark:border-neutral-800" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
...
# Part 2: Main component - state, memos, callbacks, data arrays
p2 = '''
export default function GradingDetail({dosen, student, course, gradeData: gd, attendanceRecords: ar, classAverage: ca, dosenNotes: dn}: Props) {
    ...
    const tlRecs = useMemo(() => {
        let r = [...ar];
        if (tlFilt !== 'all') r = r.filter(x => x.status === tlFilt);
        return r;
    }, [ar, tlFilt]);
    ...
    const openDetail = useCallback((r: AttRec) => { setSelected(r); setShowDet(true); }, []);
    const openEdit = useCallback((r: AttRec) => { setSelected(r); ef.setData({log_id: r.id, status: r.status, reason: ''}); setShowEdit(true); }, []);
    const saveSt = useCallback(() => { ef.post('/dosen/grading/detail/update-status', {onSuccess: () => { setShowEdit(false); ef.reset(); }}); }, []);
    const saveN = useCallback(() => { nf.post('/dosen/grading/detail/add-note', {onSuccess: () => { setShowNote(false); nf.setData({mahasiswa_id: student.id, content: '', title: ''}); }}); }, [student.id]);
    const delNote = useCallback((id: number) => { if (confirm('Yakin hapus catatan ini?')) router.delete('/dosen/grading/detail/note/' + id); }, []);

    const cards = [
        {k:'t',l:'Total Pertemuan',v:gd.total_sessions,s:'Sesi',I:BookOpen,f:'from-blue-400',t:'to-cyan-600',h:'shadow-blue-500/30'},
        {k:'h',l:'Hadir',v:sb.present,s:'Penuh',I:CheckCircle,f:'from-emerald-400',t:'to-teal-600',h:'shadow-emerald-500/30'},
        {k:'l',l:'Terlambat',v:sb.late,s:'Late',I:Clock,f:'from-amber-400',t:'to-orange-600',h:'shadow-amber-500/30'},
        {k:'i',l:'Izin/Sakit',v:sb.permit+sb.sick,s:'Keterangan',I:FileText,f:'from-blue-400',t:'to-indigo-600',h:'shadow-blue-500/30'},
        {k:'a',l:'Absen',v:sb.absent+sb.rejected,s:'Tanpa Ket.',I:XCircle,f:'from-red-400',t:'to-rose-600',h:'shadow-red-500/30'},
        {k:'p',l:'Rata-rata Poin',v:gd.average_points,s:'Per Sesi',I:Award,f:'from-purple-400',t:'to-pink-600',h:'shadow-purple-500/30'},
    ];
    const tabs: {k:string;l:string;I:any}[] = [
...
# Part 3: JSX return - header, profile, cards, grade calc
p3 = '''
    return (
        ...
        <div className="relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex flex-wrap gap-3">
                    <motion.div initial={{opacity:0,scale:0}} animate={{opacity:1,scale:1}} transition={{delay:0.5,type:'spring'}} className="flex items-center gap-3 rounded-2xl bg-white/20 backdrop-blur-xl px-5 py-3 shadow-lg border border-white/10">
                        <div className="p-2 bg-emerald-500/20 rounded-lg"><CheckCircle className="h-5 w-5" /></div>
                        <div><p className="text-xs text-indigo-100">Kehadiran</p><p className="text-xl font-bold">{gd.attended_sessions}/{gd.total_sessions}</p></div>
                    </motion.div>
                    <motion.div initial={{opacity:0,scale:0}} animate={{opacity:1,scale:1}} transition={{delay:0.6,type:'spring'}} className="flex items-center gap-3 rounded-2xl bg-white/20 backdrop-blur-xl px-5 py-3 shadow-lg border border-white/10">
                        <div className="p-2 bg-blue-500/20 rounded-lg"><TrendingUp className="h-5 w-5" /></div>
                        <div><p className="text-xs text-indigo-100">Persentase</p><p className="text-xl font-bold">{gd.attendance_rate}%</p></div>
                    </motion.div>
            ...
            <div className="flex flex-wrap gap-3 mt-6 pt-5 border-t border-white/10">
                <motion.button whileHover={{scale:1.02}} whileTap={{scale:0.98}} onClick={() => setShowExp(true)} className="flex items-center gap-2 rounded-xl bg-white/20 px-5 py-2.5 text-sm font-semibold backdrop-blur-md border border-white/20 shadow-lg hover:bg-white/30"><Download className="h-4 w-4" /> Export</motion.button>
    ...
    <motion.div variants={cV} className="grid gap-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {cards.map(c => (
            <motion.div key={c.k} variants={iV} whileHover={{scale:1.03,y:-8}} className="relative overflow-hidden rounded-3xl border border-white/20 bg-white/40 dark:bg-neutral-900/40 p-5 shadow-xl backdrop-blur-xl">
                <div className="flex items-center gap-3">
                    <div className={'flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br text-white shadow-lg ' + c.f + ' ' + c.t + ' ' + c.h}><c.I className="h-6 w-6" /></div>
                    <div><p className="text-2xl font-bold text-neutral-900 dark:text-white">{c.v}</p><p className="text-xs text-neutral-500">{c.s}</p></div>
                </div>
