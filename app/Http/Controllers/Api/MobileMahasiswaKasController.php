...
class MobileMahasiswaKasController extends Controller
{
    ...
    /**
     * Main dashboard — returns all kas data in one call.
     */
    public function index(Request $request): JsonResponse
    {
        ...
        $totalPaid = (float) Kas::where('mahasiswa_id', $mahasiswa->id)
            ->where('type', 'income')->where('status', 'paid')->sum('amount');
        $totalUnpaid = (float) Kas::where('mahasiswa_id', $mahasiswa->id)
            ->where('type', 'income')->where('status', 'unpaid')->sum('amount');
        $paidCount = Kas::where('mahasiswa_id', $mahasiswa->id)
            ->where('type', 'income')->where('status', 'paid')->count();
        $unpaidCount = Kas::where('mahasiswa_id', $mahasiswa->id)
            ->where('type', 'income')->where('status', 'unpaid')->count();
        ...
        if (Schema::hasTable('kas_payment_receipts')) {
            $receiptWorkflow = DB::table('kas_payment_receipts as r')
                ...
                ->limit(12)
                ->get([
                    'r.id', 'r.kas_id', 'r.status', 'r.image_url',
                    'r.ocr_data', 'r.created_at', 'r.reviewed_at',
                    'k.amount as expected_amount', 'k.description as kas_description', 'k.period_date',
                ])
                ->map(function ($row) {
    ...
    /**
     * Upload a receipt image (bukti transfer).
     */
    public function uploadReceipt(Request $request): JsonResponse
    {
        ...
        if (! Schema::hasTable('kas_payment_receipts')) {
            return response()->json([
                'message' => 'Tabel kas_payment_receipts belum tersedia.',
            ], 503);
        }
        ...
        $receiptId = DB::table('kas_payment_receipts')->insertGetId([
            ...
            'image_url' => $path,
            'ocr_data' => json_encode(['amount' => null, 'date' => null, 'bankName' => null, 'confidence' => 0]),
            'status' => 'pending',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
