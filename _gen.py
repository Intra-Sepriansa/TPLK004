...
import {
    ...
    BarChart3, MessageSquare, BookOpen, XCircle, RefreshCw, Plus, Trash2,
    ArrowUpDown, Target, Calculator, Eye
} from 'lucide-react';
...
interface Props {
    ...
    student: {id: number; nama: string; nim: string; email: string; foto?: string|null;
              prodi: string; fakultas: string; semester: number; kelas?: string|null; angkatan?: string|null};
    course: {id: number; nama: string; kode: string; sks: number; semester: string; tahun_ajaran: string};
    gradeData: {
        total_sessions: number; attended_sessions: number; attendance_rate: number;
        average_points: number; attendance_grade: number; grade_letter: string;
...
const sC = (s: string): string => ({
    present:'text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400',
    ...
    rejected:'text-rose-600 bg-rose-100 dark:bg-rose-900/30 dark:text-rose-400',
} as Record<string,string>)[s] || 'text-neutral-600 bg-neutral-100';
...
export default function GradingDetail({dosen, student, course, gradeData: gd, attendanceRecords: ar, classAverage: ca, dosenNotes: dn}: Props) {
    ...
    const [tlFilt, setTlFilt] = useState('all');
    ...
    const filtered = useMemo(() => {
        ...
        if (fStat !== 'all') r = r.filter(x => x.status === fStat);
        r.sort((a, b) => sortOrd === 'asc' ? a.meeting_number - b.meeting_number : b.meeting_number - a.meeting_number);
        return r;
    }, [ar, searchQ, fStat, sortOrd]);

    const lineData = useMemo(() => ar.map(r => ({name: 'P'+r.meeting_number, poin: r.points})), [ar]);
    const pieData = useMemo(() => [
        {name:'Hadir',value:sb.present,color:COLORS.present},{name:'Terlambat',value:sb.late,color:COLORS.late},
        {name:'Izin',value:sb.permit,color:COLORS.permit},{name:'Sakit',value:sb.sick,color:COLORS.sick},
        {name:'Absen',value:sb.absent,color:COLORS.absent},{name:'Ditolak',value:sb.rejected,color:COLORS.rejected},
    ...
    const tlRecs = useMemo(() => { let r = [...ar]; if (tlFilt !== 'all') r = r.filter(x => x.status === tlFilt); return r; }, [ar, tlFilt]);
    ...
    const openDet = useCallback((r: AttRec) => { setSel(r); setShowDet(true); }, []);
    const openEdit = useCallback((r: AttRec) => { setSel(r); ef.setData({log_id: r.id, status: r.status, reason: ''}); setShowEdit(true); }, []);
    const saveSt = useCallback(() => { ef.post('/dosen/grading/detail/update-status', {onSuccess: () => { setShowEdit(false); ef.reset(); }}); }, []);
    const saveNote = useCallback(() => { nf.post('/dosen/grading/detail/add-note', {onSuccess: () => { setShowNote(false); nf.setData({mahasiswa_id: student.id, content: '', title: ''}); }}); }, [student.id]);
    const delNote = useCallback((id: number) => { if (confirm('Yakin hapus catatan ini?')) router.delete('/dosen/grading/detail/note/' + id); }, []);

    /* ── Card & Tab data ── */
    const cards = [
        {k:'t',l:'Total Pertemuan',v:gd.total_sessions,s:'Sesi',I:BookOpen,f:'from-blue-400',t2:'to-cyan-600',sh:'shadow-blue-500/30'},
        {k:'h',l:'Hadir',v:sb.present,s:'Penuh',I:CheckCircle,f:'from-emerald-400',t2:'to-teal-600',sh:'shadow-emerald-500/30'},
        {k:'l',l:'Terlambat',v:sb.late,s:'Late',I:Clock,f:'from-amber-400',t2:'to-orange-600',sh:'shadow-amber-500/30'},
        {k:'i',l:'Izin/Sakit',v:sb.permit+sb.sick,s:'Keterangan',I:FileText,f:'from-blue-400',t2:'to-indigo-600',sh:'shadow-blue-500/30'},
        {k:'a',l:'Absen',v:sb.absent+sb.rejected,s:'Tanpa Ket.',I:XCircle,f:'from-red-400',t2:'to-rose-600',sh:'shadow-red-500/30'},
        {k:'p',l:'Rata-rata Poin',v:gd.average_points,s:'Per Sesi',I:Award,f:'from-purple-400',t2:'to-pink-600',sh:'shadow-purple-500/30'},
    ];
    const tabList: {k:string;l:string;I:any}[] = [
        {k:'riwayat',l:'Riwayat',I:FileText},{k:'grafik',l:'Grafik',I:BarChart3},
        {k:'timeline',l:'Timeline',I:Calendar},{k:'catatan',l:'Catatan',I:MessageSquare},
    ...
    <motion.div variants={iV} className="relative overflow-hidden rounded-3xl p-6 md:p-8 text-white shadow-2xl">
        ...
        {[0,1,2].map(i => (
            <motion.div key={i} className="absolute right-16 top-1/2 -translate-y-1/2 h-32 w-32 rounded-full border-2 border-white/10"
                animate={{scale:[1,2.5],opacity:[0.4,0]}} transition={{duration:3,repeat:Infinity,ease:'easeOut',delay:i}} />
        ))}
        <div className="relative">
            <div className="flex items-center gap-2 mb-6">
                <motion.button whileHover={{scale:1.05,x:-3}} whileTap={{scale:0.95}}
                    onClick={() => router.visit('/dosen/grading')}
                    className="flex items-center gap-2 rounded-xl bg-white/20 px-4 py-2 text-sm font-semibold backdrop-blur-md border border-white/20 shadow-lg hover:bg-white/30">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                    <motion.div whileHover={{scale:1.1,rotate:5}}
                        className="flex h-20 w-20 items-center justify-center rounded-2xl bg-white/20 backdrop-blur-xl border border-white/30 text-3xl font-bold shadow-xl">
                        {student.foto ? <img src={student.foto} alt="" className="h-full w-full rounded-2xl object-cover" /> : ini(student.nama)}
                    </motion.div>
                    <div>
                ...
                <div className="flex flex-wrap gap-3">
                    {[
                        {delay:0.5,icon:<CheckCircle className="h-5 w-5"/>,bg:'bg-emerald-500/20',label:'Kehadiran',val:`${gd.attended_sessions}/${gd.total_sessions}`},
                        {delay:0.6,icon:<TrendingUp className="h-5 w-5"/>,bg:'bg-blue-500/20',label:'Persentase',val:`${gd.attendance_rate}%`},
                    ].map((st,idx) => (
                        <motion.div key={idx} initial={{opacity:0,scale:0}} animate={{opacity:1,scale:1}} transition={{delay:st.delay,type:'spring'}}
                            className="flex items-center gap-3 rounded-2xl bg-white/20 backdrop-blur-xl px-5 py-3 shadow-lg border border-white/10">
                            <div className={`p-2 ${st.bg} rounded-lg`}>{st.icon}</div>
                            <div><p className="text-xs text-indigo-100">{st.label}</p><p className="text-xl font-bold">{st.val}</p></div>
                        </motion.div>
                    ))}
                    <motion.div initial={{opacity:0,scale:0}} animate={{opacity:1,scale:1}} transition={{delay:0.7,type:'spring'}}
                        className="flex items-center gap-3 rounded-2xl bg-white/20 backdrop-blur-xl px-5 py-3 shadow-lg border border-white/10">
                        <div className={`px-4 py-2 rounded-xl text-white text-2xl font-bold shadow-lg ${gCol(gd.grade_letter)}`}>{gd.grade_letter}</div>
                    </motion.div>
                </div>
            </div>
            <div className="flex flex-wrap gap-3 mt-6 pt-5 border-t border-white/10">
                {[
                    {fn:() => setShowExp(true),icon:<Download className="h-4 w-4"/>,label:'Export'},
                    {fn:() => window.print(),icon:<Printer className="h-4 w-4"/>,label:'Print'},
                    {fn:() => router.reload(),icon:<RefreshCw className="h-4 w-4"/>,label:'Refresh'},
                ].map((btn,idx) => (
                    <motion.button key={idx} whileHover={{scale:1.02}} whileTap={{scale:0.98}} onClick={btn.fn}
                        className="flex items-center gap-2 rounded-xl bg-white/20 px-5 py-2.5 text-sm font-semibold backdrop-blur-md border border-white/20 shadow-lg hover:bg-white/30">
                        {btn.icon} {btn.label}
                    </motion.button>
                ))}
            </div>
    ...
    {/* ── PROFILE ── */}
    <motion.div variants={iV} className="rounded-3xl border border-white/20 bg-white/50 dark:bg-neutral-900/50 p-6 shadow-lg backdrop-blur-xl dark:border-neutral-800">
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs font-semibold">
                            <CheckCircle className="h-3 w-3" />Aktif
                        </span>
                        <span className={'inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-semibold ' +
                            (gd.can_take_uas ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400')}>
    ...
    {/* ── 6 SUMMARY CARDS ── */}
    <motion.div variants={cV} className="grid gap-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {cards.map(c => (
            <motion.div key={c.k} variants={iV} whileHover={{scale:1.03,y:-8}}
                className="relative overflow-hidden rounded-3xl border border-white/20 bg-white/40 dark:bg-neutral-900/40 p-5 shadow-xl backdrop-blur-xl">
                <div className="flex items-center gap-3">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br text-white shadow-lg ${c.f} ${c.t2} ${c.sh}`}>
                        <c.I className="h-6 w-6" />
                    </div>
                    <div>
                        <p className="text-2xl font-bold text-neutral-900 dark:text-white">{c.v}</p>
                        <p className="text-xs text-neutral-500">{c.s}</p>
                    </div>
                </div>
                <p className="text-xs font-medium text-neutral-600 dark:text-neutral-300 mt-3">{c.l}</p>
            </motion.div>
        ))}
    </motion.div>
    ...
    <motion.div variants={iV} className="rounded-3xl border border-white/20 bg-white/50 dark:bg-neutral-900/50 p-6 shadow-lg backdrop-blur-xl dark:border-neutral-800">
        <h3 className="text-lg font-bold text-neutral-900 dark:text-white flex items-center gap-2 mb-4">
            <Calculator className="h-5 w-5 text-purple-500" />Rincian Perhitungan Nilai
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="rounded-2xl bg-neutral-50 dark:bg-neutral-800/50 p-4">
                ...
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-white font-bold text-xl ${gCol(gd.grade_letter)}`}>{gd.grade_letter}</div>
            ...
            <div className="rounded-2xl bg-neutral-50 dark:bg-neutral-800/50 p-4">
                ...
                {gd.can_take_uas ? (
                    ...
                    <div>
                        <div className="flex items-center gap-2 text-red-600"><XCircle className="h-6 w-6" /><span className="font-bold">Tidak Bisa UAS</span></div>
                        {gd.sessions_needed_for_uas > 0 && <p className="text-sm text-neutral-500 mt-1">Butuh {gd.sessions_needed_for_uas} sesi lagi</p>}
                    </div>
                )}
            </div>
        </div>
    ...
    {/* ── TABS CONTAINER ── */}
    <motion.div variants={iV} className="rounded-3xl border border-white/20 bg-white/50 dark:bg-neutral-900/50 shadow-lg backdrop-blur-xl dark:border-neutral-800 overflow-hidden">
        {/* Tab bar */}
        <div className="flex overflow-x-auto border-b border-neutral-200 dark:border-neutral-800 p-1
