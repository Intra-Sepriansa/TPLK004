 ...
 */
export async function saveOfflineAttendance(data: Omit<OfflineAttendance, 'id' | 'timestamp'>): Promise<void> {
    try {
        const queue: OfflineAttendance[] = await localforage.getItem(STORE_KEY) || [];
...
export async function syncOfflineAttendances(): Promise<void> {
    ...
    const syncToastId = toast.loading(`Mensinkronkan ${queue.length} absen offline...`);
    ...
    for (const item of queue) {
        try {
            ...
            const bestSample = item.location_samples.reduce((a, b) => a.accuracy_m < b.accuracy_m ? a : b);
            formData.append('location_captured_at', bestSample.captured_at);

            item.location_samples.forEach((sample, index) => {
                formData.append(`location_samples[${index}][latitude]`, sample.latitude.toString());
                formData.append(`location_samples[${index}][longitude]`, sample.longitude.toString());
                formData.append(`location_samples[${index}][accuracy_m]`, sample.accuracy_m.toString());
                formData.append(`location_samples[${index}][captured_at]`, sample.captured_at);
            });
            ...
            if (item.selfiePreview) {
                const selfieFile = dataURLtoFile(item.selfiePreview, `offline-selfie-${item.timestamp}.jpg`);
            ...
            if (error.response && error.response.status >= 400 && error.response.status < 500 && error.response.status !== 429) {
                ...
                await removeOfflineAttendance(item.id);
                console.log(`Dropped invalid offline attendance ${item.id} due to ${error.response.status}`);
            } else {
                failCount++;
            }
        }
    ...
    if (successCount > 0 && failCount === 0) {
        ...
        toast.warning(`${successCount} sinkron, ${failCount} gagal tertunda. Berkas akan dicoba kembali nanti.`);
    } else if (failCount > 0) {
        toast.error(`Gagal sinkron absen offline. Menunggu koneksi yang lebih stabil.`);
    }
}
