...
export default function CatatanDetail({ note, relatedNotes }: Props) {
    ...
    const { contentWithAnchors, tocItems } = useMemo(() => {
        if (typeof window === 'undefined') {
            return { contentWithAnchors: note.content, tocItems: [] as TocItem[] };
        }
            ...
            <div className="p-4 md:p-6">
                <div className="mx-auto max-w-7xl space-y-6">
                    ...
                    >
                        ...
                        />
                        <div className="absolute -right-14 -top-14 h-56 w-56 rounded-full bg-white/15 blur-3xl" />
                        <div className="absolute -left-16 bottom-0 h-44 w-44 rounded-full bg-fuchsia-300/20 blur-3xl" />
                            ...
                            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                                <div className="space-y-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        />
                                        <div>
                                            <p className="text-sm font-medium text-indigo-100">Detail Catatan Akademik</p>
                                            <h1 className="text-2xl font-extrabold sm:text-4xl">{note.title}</h1>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-2.5">
                                        ...
                                        <Badge className="border-white/30 bg-white/15 text-white">
                                            ...
                                            Pertemuan {note.meeting_number}/{note.total_meetings}
                                        </Badge>
                                        <Badge className="border-white/30 bg-white/15 text-white">
                                        ...
                                        {note.sks ? (
                                            <Badge className="border-white/30 bg-white/15 text-white">{note.sks} SKS</Badge>
                                        ) : null}
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                                    <Button
                                        type="button"
                                        onClick={() => router.visit(`/user/akademik/catatan/${note.id}/edit`)}
                                    ...
                                    <a href={`/user/akademik/catatan/${note.id}/export-pdf`}>
                                        <Button
                                            type="button"
                    ...
                    <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                        {[
                            { label: 'Jumlah Kata', value: note.word_count, icon: FileText, color: 'text-sky-500' },
                            { label: 'Estimasi Baca', value: `${note.reading_time} menit`, icon: Clock3, color: 'text-violet-500' },
                            { label: 'Referensi Link', value: note.links.length, icon: Link2, color: 'text-emerald-500' },
                            { label: 'Tag Topik', value: note.tags.length, icon: Tag, color: 'text-amber-500' },
                        ].map((stat, index) => (
                            <motion.div
                                key={stat.label}
                            ...
                            >
                                <div className="flex items-start justify-between gap-2">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xl font-bold text-neutral-900 dark:text-white">{stat.value}</p>
                                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
                        <motion.section
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            className="xl:col-span-8 rounded-3xl border border-white/20 bg-white/40 p-5 shadow-xl backdrop-blur-xl dark:border-white/5 dark:bg-neutral-900/40 sm:p-7"
                        >
                            <div className="mb-5 flex items-center justify-between gap-3 border-b border-neutral-200/70 pb-4 dark:border-neutral-700/70">
                                <div>
                                    <h2 className="text-lg font-bold text-neutral-900 dark:text-white">Isi Catatan</h2>
                            ...
                            <article
                                className="prose prose-neutral max-w-none leading-relaxed dark:prose-invert prose-a:text-indigo-600 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl prose-pre:rounded-xl"
                        ...
                        <div className="space-y-6 xl:col-span-4">
                            ...
                            >
                                <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-neutral-600 dark:text-neutral-300">
                                    Navigasi Konten
                                </h3>
                                {tocItems.length > 0 ? (
                                        ...
                                        {tocItems.map((item) => (
                                            <a
                                                ...
                                                className={cn(
                                                    ...
                                                    headingClassMap[item.level] ?? headingClassMap[3],
                                                )}
                                            >
                                                {item.text}
                                            </a>
                                    ...
                                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                                        Belum ada heading terstruktur pada konten ini.
                                    </p>
                                )}
                            </motion.aside>
                            ...
                            <motion.aside
                                ...
                                className="rounded-3xl border border-white/20 bg-white/40 p-5 shadow-xl backdrop-blur-xl dark:border-white/5 dark:bg-neutral-900/40"
                            >
                                <h3 className="mb-3 flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-neutral-600 dark:text-neutral-300">
                                    <Sparkles className="h-4 w-4 text-violet-500" />
                                    Insight AI
                                </h3>
                                {note.ai_summary ? (
                                    <p className="text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">{note.ai_summary}</p>
                                ) : (
                                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                                        Ringkasan AI belum tersedia untuk catatan ini.
                                    </p>
                                )}
                    ...
                    {relatedNotes.length > 0 ? (
                        ...
                        >
                            <div className="mb-4 flex items-center justify-between gap-3">
                                <h3 className="text-lg font-bold text-neutral-900 dark:text-white">Catatan Terkait</h3>
                                <Link
                                    href="/user/akademik/catatan"
                            ...
                            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                                {relatedNotes.map((related) => (
                                    <button
                                        type="button"
                                        key={related.id}
                                        onClick={() => router.visit(`/user/akademik/catatan/${related.id}`)}
                                    ...
                                    >
                                        ...
                                        <div className="mt-3 flex items-center justify-between text-xs text-neutral-500 dark:text-neutral-400">
                                            <span>Pertemuan {related.meeting_number}</span>
