...
export default function KasVotingCreate({ mahasiswa, stats, defaults }: Props) {
    ...
    const selectedCategory = categories.find((c) => c.value === data.category) ?? categories[0];
                    ...
                    <div className="relative">
                        ...
                        <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-start sm:gap-6 sm:text-left">
                            ...
                            <div className="mt-1 flex-1">
                                <p className="text-sm font-medium tracking-wide text-indigo-100">Keuangan Kelas</p>
                                <h1 className="mt-1 text-2xl font-bold sm:text-3xl">Usulkan Pengeluaran Kas</h1>
                                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-indigo-100 sm:text-base">
                                    Buat usulan pengeluaran kas dengan detail yang jelas agar seluruh anggota kelas bisa menilai dan voting secara adil.
                                </p>
                ...
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
                    ...
                    >
                        <div className="mb-5 flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-lg font-semibold text-neutral-900 dark:text-white">Form Usulan</h2>
                                <p className="text-sm text-neutral-500 dark:text-neutral-400">
                                    Lengkapi data berikut untuk membuat voting pengeluaran baru
                                </p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-neutral-400">Maksimal 1000 karakter</span>
                                    <span className="text-xs text-neutral-400">{data.description.length}/1000</span>
                                </div>
                                {errors.description && (
                                    <p className="flex items-center gap-1 text-xs text-red-500">
                            ...
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    {errors.amount && (
                                        <p className="flex items-center gap-1 text-xs text-red-500">
                                            <AlertCircle className="h-3.5 w-3.5" /> {errors.amount}
                                        </p>
                                    )}
                                </div>
                            ...
                            <div className="rounded-2xl border border-white/20 bg-white/50 p-4 backdrop-blur-xl dark:border-white/10 dark:bg-neutral-800/50">
                                <p className="mb-2 text-sm font-semibold text-neutral-800 dark:text-neutral-200">Catatan Penting</p>
                                <ul className="space-y-1 text-xs text-neutral-600 dark:text-neutral-300">
                                    <li>• Voting aktif selama {defaults.voting_duration_days} hari setelah usulan dibuat.</li>
                                    <li>• Minimal {defaults.min_votes} vote untuk hasil dianggap valid.</li>
                                    <li>• Ambang persetujuan minimal {defaults.approval_threshold}%.</li>
                                </ul>
                            </div>
                            ...
                            <div className="flex flex-col gap-3 pt-2 sm:flex-row">
                                ...
                                >
                                    {processing ? 'Menyimpan...' : 'Kirim Usulan'}
                                </Button>
                            </div>
                        </form>
                    </motion.div>
                    ...
                    >
                        <div className="mb-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-neutral-500 dark:text-neutral-400">Ringkasan usulan Anda</p>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <div className="rounded-2xl border border-white/20 bg-white/50 p-4 dark:border-white/10 dark:bg-neutral-800/50">
                                <p className="text-xs text-neutral-500 dark:text-neutral-400">Judul</p>
                            ...
                            <div className="rounded-2xl border border-white/20 bg-white/50 p-4 dark:border-white/10 dark:bg-neutral-800/50">
                                <p className="text-xs text-neutral-500 dark:text-neutral-400">Nominal</p>
                                <p className="mt-1 text-xl font-bold bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent">
                                    {formatCurrency(Number.isNaN(amountNumber) ? 0 : amountNumber)}
                                </p>
                            </div>

                            <div className="rounded-2xl border border-white/20 bg-white/50 p-4 dark:border-white/10 dark:bg-neutral-800/50">
                                <p className="mb-2 text-xs text-neutral-500 dark:text-neutral-400">Statistik Voting Saat Ini</p>
                                <div className="grid grid-cols-3 gap-2 text-center">
                                    <div>
                                        <p className="text-lg font-bold text-blue-600 dark:text-blue-400">{stats.open}</p>
                                        <p className="text-[10px] text-neutral-500">Aktif</p>
                                    </div>
                                    <div>
                                        <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{stats.approved}</p>
                                        <p className="text-[10px] text-neutral-500">Setuju</p>
                                    </div>
                                    <div>
                                        <p className="text-lg font-bold text-red-600 dark:text-red-400">{stats.rejected}</p>
                                        <p className="text-[10px] text-neutral-500">Tolak</p>
                                    </div>
                                </div>
                                <p className="mt-2 text-[10px] text-neutral-500">Total usulan: {totalVotings}</p>
                            </div>

                            <div className="rounded-2xl border border-white/20 bg-white/50 p-4 dark:border-white/10 dark:bg-neutral-800/50">
                                <p className="mb-1 inline-flex items-center gap-1 text-xs font-semibold text-neutral-700 dark:text-neutral-300">
                                    <Clock className="h-3.5 w-3.5" /> Deadline Otomatis
                                </p>
                                <p className="text-xs text-neutral-500 dark:text-neutral-400">
                                    Voting otomatis dibuka {defaults.voting_duration_days} hari dan ditutup oleh sistem saat waktunya habis.
                                </p>
                            </div>
                        </div>
                    </motion.div>
