...
function FormSection({
    ...
    title: string; description: string; icon: any; gradient: string; children: ReactNode;
}) {
    return (
        <motion.div
...
function FormField({
    label, icon: Icon, required, optional, error, helper, locked, lockMessage, children, className,
}: {
    label: string; icon: any; required?: boolean; optional?: boolean;
    error?: string; helper?: string; locked?: boolean; lockMessage?: string;
    ...
    return (
        <div className={cn('space-y-2', className)}>
            ...
            {children}
            {lockMessage && (
                <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1 italic">
...
function InfoItem({ icon: Icon, label, value, color }: { icon: any; label: string; value?: string | number; color: string }) {
...
function ConflictChecker({
    startAt, endAt, existingSessions,
}: {
    startAt: string; endAt: string; existingSessions: ExistingSession[];
}) {
    if (!startAt || !endAt) return null;
        ...
        return (
            ...
            >
                ...
                <div className="flex-1">
                    ...
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">Waktu ini tersedia</p>
                </div>
            </motion.div>
        );
    }
                            ...
                            return (
                                <div key={c.id} className="p-2 rounded-lg bg-rose-100 dark:bg-rose-900/30">
                                    <p className="text-xs font-medium text-rose-800 dark:text-rose-200">
                                        Pertemuan {c.meeting_number}{c.title && ` — ${c.title}`}
                                    </p>
                                    <p className="text-xs text-rose-600 dark:text-rose-400">
                                        {cStart.toLocaleString('id-ID', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })} - {cEnd.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                                    </p>
                                </div>
                            );
                        })}
...
function ChangeTracker({ changes }: { changes: Change[] }) {
    ...
    return (
        <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-4"
        >
            <div className="flex items-start gap-3">
                    ...
                    <div className="mt-2 space-y-1.5">
                        {changes.map((c, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs">
                                <span className="font-medium text-amber-800 dark:text-amber-200">{c.field}:</span>
                                <span className="text-rose-600 dark:text-rose-400 line-through">{c.oldValue}</span>
                                <span className="text-neutral-400">→</span>
                                <span className="text-emerald-600 dark:text-emerald-400 font-medium">{c.newValue}</span>
                            </div>
                        ))}
                    </div>
                </div>
...
export default function EditJadwal({ session: schedule, courses, existingSessions }: PageProps) {
    ...
    const hasTimeChanged = useMemo(
        () => form.data.start_at !== originalData.start_at || form.data.end_at !== originalData.end_at,
    ...
    const getChanges = (): Change[] => {
        ...
        const formatVal = (key: string, val: any): string => {
            ...
            if (key === 'start_at' || key === 'end_at') {
                return val ? new Date(val).toLocaleString('id-ID', { dateStyle: 'short', timeStyle: 'short' }) : '-';
            }
            return String(val || '-');
        };

        return (Object.keys(form.data) as Array<keyof typeof form.data>)
            .filter(key => String(form.data[key]) !== String(originalData[key]))
            .map(key => ({
    ...
    const disabledClass = 'w-full rounded-xl bg-neutral-100 dark:bg-neutral-800/30 border border-neutral-200 dark:border-neutral-700 px-4 py-3 text-neutral-500 dark:text-neutral-500 cursor-not-allowed text-sm';
    ...
    const origTimeStr = origStart && origEnd
            ...
            <div className="p-6 space-y-6">
                    ...
                    <div className="relative">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-4">
                            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-5 text-center sm:text-left w-full">
                                ...
                                >
                                    <img src={JadwalIcon} alt="Edit Jadwal" className="absolute inset-0 h-full w-full object-contain drop-shadow-2xl" />
                                </motion.div>
                ...
                >
                    <div className="flex items-start justify-between mb-4">
                        <div>
                            <h2 className="text-lg font-bold text-neutral-900 dark:text-white">Informasi Jadwal Saat Ini</h2>
                            <p className="text-sm text-neutral-500 dark:text-neutral-400">Data yang sedang aktif untuk jadwal ini</p>
                        </div>
                        <span className={cn(
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <InfoItem icon={BookOpen} label="Mata Kuliah" value={schedule.course?.nama} color="indigo" />
                        <InfoItem icon={Hash} label="Pertemuan" value={`#${schedule.meeting_number}`} color="purple" />
                        <InfoItem icon={Clock} label="Waktu" value={origTimeStr} color="pink" />
                        <InfoItem icon={Users} label="Kehadiran" value={`${schedule.logs_count || 0} mahasiswa`} color="emerald" />
                    </div>
                </motion.div>
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <FormField
                                ...
                                lockMessage={courseIsLocked ? 'Tidak dapat diubah karena sudah ada data kehadiran' : undefined}
                            >
                                {courseIsLocked ? (
                                    <div className={disabledClass}>
                                        {schedule.course?.nama} ({schedule.course?.sks} SKS)
                                    </div>
                                ) : (
                                    <select
                            ...
                            <FormField label="Pertemuan Ke" icon={Hash} required error={form.errors.meeting_number}>
                                <input
                                    type="number"
                                    min={1}
                                    max={16}
                            ...
                            <FormField label="Judul/Topik Pertemuan" icon={Type} optional className="md:col-span-2">
                                <input
                                    type="text"
                    ...
                    >
                        ...
                        {timeIsLocked && (
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="mb-4 p-4 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 flex items-start gap-3"
                            >
                                <Lock className="h-5 w-5 text-amber-600 dark:text-amber-400 mt-0.5 shrink-0" />
                                <div>
                                    <p className="text-sm font-medium text-amber-900 dark:text-amber-100">Jadwal Sedang Aktif</p>
                                    <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                                        Waktu tidak dapat diubah karena jadwal ini sedang berjalan. Nonaktifkan terlebih dahulu untuk mengubah waktu.
                                    </p>
                                </div>
                            </motion.div>
                        )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <FormField
                                ...
                                locked={timeIsLocked}
                                lockMessage={timeIsLocked ? 'Tidak dapat diubah saat jadwal aktif' : undefined}
                            >
                                <input
                                    type="datetime-local"
                                    ...
                                    className={cn(timeIsLocked ? disabledClass : inputClass, '[color-scheme:dark]')}
                                />
                            </FormField>
                        ...
                        >
                            ...
                            >
                                <div className="relative p-6 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 text-white">
                                    ...
                                    <h3 className="text-2xl font-bold">Preview Perubahan</h3>
                    ...
                    {showSuccess && (
                        ...
                        >
                            ...
                            >
                                ...
                                <p className="text-neutral-400 text-lg">Jadwal berhasil diperbarui</p>
