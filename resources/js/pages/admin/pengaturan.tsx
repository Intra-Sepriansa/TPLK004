...
const tabs: Array<{
...
}> = [
    {
        ...
        label: 'Umum',
        icon: Settings,
        keywords: ['token', 'ttl', 'selfie', 'geofence', 'tema'],
    },
    {
        id: 'security',
        label: 'Keamanan Akun',
        icon: ShieldCheck,
        keywords: ['password', '2fa', 'sesi', 'keamanan'],
    },
    {
        id: 'notifications',
        label: 'Notifikasi',
        icon: Bell,
        keywords: ['email', 'push', 'laporan'],
    },
    {
        id: 'templates',
        label: 'Template Email',
        icon: Mail,
        keywords: ['template', 'email', 'body', 'subject'],
    },
    {
        id: 'advanced',
        label: 'Lanjutan',
        icon: Zap,
        keywords: ['ai', 'maintenance', 'threshold', 'login'],
    },
    {
        id: 'system',
        label: 'Sistem',
        icon: Server,
        keywords: ['server', 'storage', 'backup', 'history', 'cache'],
    },
];
...
export default function AdminPengaturan({
    ...
    const matchesSearch = (
        ...terms: Array<string | number | boolean | null | undefined>
    ) =>
        search.length === 0 ||
        terms.some((term) =>
            String(term ?? '')
    ...
    const quickStats = [
        ...
        },
        {
            ...
            value: stats.last_backup_at
                ? formatDate(stats.last_backup_at, 'date')
                : 'Belum ada',
            sub: stats.last_backup_at
                ? formatDate(stats.last_backup_at, 'time')
                : 'Buat backup pertama',
    ...
    const submitGeneral = (e: FormEvent) => {
        ...
        generalForm.patch('/admin/pengaturan', {
            preserveScroll: true,
            onSuccess: () =>
                pushToast('success', 'Pengaturan umum berhasil disimpan.'),
            onError: () =>
                pushToast('error', 'Gagal menyimpan pengaturan umum.'),
        });
    };
    ...
    const submitGeofence = (e: FormEvent) => {
        ...
        geofenceForm.patch('/admin/pengaturan/geofence', {
            preserveScroll: true,
            onSuccess: () =>
                pushToast('success', 'Pengaturan geofence berhasil disimpan.'),
            onError: () =>
                pushToast('error', 'Gagal menyimpan pengaturan geofence.'),
        });
    };
    ...
    const submitNotifications = (e: FormEvent) => {
        ...
        notificationForm.patch('/admin/pengaturan/notifications', {
            ...
            onError: () =>
                pushToast('error', 'Gagal menyimpan pengaturan notifikasi.'),
        });
    };
    ...
    const submitAdvanced = (e: FormEvent) => {
        ...
        advancedForm.patch('/admin/pengaturan/advanced', {
            preserveScroll: true,
            onSuccess: () =>
                pushToast('success', 'Pengaturan lanjutan berhasil disimpan.'),
            onError: () =>
                pushToast('error', 'Gagal menyimpan pengaturan lanjutan.'),
        });
    };
    ...
    const submitTemplate = (e: FormEvent) => {
        ...
        templateForm.patch(
            `/admin/pengaturan/templates/${editingTemplate.id}`,
            {
                preserveScroll: true,
                onSuccess: () => {
                ...
                onError: () =>
                    pushToast('error', 'Gagal memperbarui template.'),
            },
        );
    };
    ...
    const handleRestoreBackup = (backupId: number) => {
        if (
            !window.confirm(
                'Restore backup ini akan mengganti pengaturan saat ini. Lanjutkan?',
            )
        ) {
            return;
        }
        ...
        router.post(
            `/admin/pengaturan/backup/${backupId}/restore`,
            {},
            {
                preserveScroll: true,
                onSuccess: () =>
                    pushToast('success', 'Backup berhasil di-restore.'),
    ...
    const handleImportFile = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        router.post(
            '/admin/pengaturan/import',
            { file },
            {
                preserveScroll: true,
                forceFormData: true,
                onSuccess: () =>
                    pushToast('success', 'Pengaturan berhasil diimport.'),
                onError: () =>
                    pushToast('error', 'Gagal mengimport pengaturan.'),
                onFinish: () => {
    ...
    const clearCache = () => {
        router.post(
            '/admin/pengaturan/clear-cache',
            {},
            {
                preserveScroll: true,
                onSuccess: () =>
                    pushToast('success', 'Cache berhasil dibersihkan.'),
    ...
    const optimizeSystem = () => {
        router.post(
            '/admin/pengaturan/optimize',
            {},
            {
                preserveScroll: true,
                onSuccess: () =>
                    pushToast('success', 'Sistem berhasil dioptimasi.'),
        ...
        <AppLayout>
            ...
            >
                ...
                <AnimatePresence>
                    {activeToast && (
                        <motion.div
                            ...
                            className={`fixed top-6 right-6 z-50 flex items-center gap-3 rounded-2xl border px-4 py-3 shadow-2xl backdrop-blur-md ${
                ...
                >
                    ...
                    <div className="absolute top-0 -right-20 h-56 w-56 rounded-full bg-white/10 blur-3xl" />
                        ...
                        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                            <div className="flex items-center gap-5">
                                <img
                                    src={PengaturanIcon}
                                    alt="Pengaturan"
                                    className="h-20 w-20 shrink-0 object-contain drop-shadow-[0_16px_28px_rgba(0,0,0,0.35)]"
                                />
                                <div>
                                    <p className="text-sm font-semibold tracking-[0.24em] text-indigo-100 uppercase">
                                    ...
                                    <p className="mt-2 max-w-2xl text-sm leading-relaxed text-indigo-100/90">
                                        Kelola konfigurasi absensi, notifikasi,
                                        keamanan, dan utility sistem dari satu
                ...
                >
                    <div className="relative w-full max-w-xl">
                        ...
                        <Input
                            ...
                            className="h-12 rounded-2xl border-slate-200 bg-white pr-4 pl-12 shadow-sm dark:border-neutral-600 dark:bg-neutral-900"
                        />
                    </div>
                ...
                <motion.section
                    variants={itemVariants}
                    className="flex gap-2 overflow-x-auto rounded-2xl border border-slate-200/70 bg-white/70 p-1.5 shadow-lg backdrop-blur-xl dark:border-neutral-600/70 dark:bg-neutral-900/70"
                >
                    {tabs.map((tab) => {
                        const Icon = tab.icon;
                ...
                <motion.section
                    variants={itemVariants}
                    className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/70 bg-white/70 px-5 py-4 shadow-lg backdrop-blur-xl dark:border-neutral-600/70 dark:bg-neutral-900/70"
                >
                    <div>
                        ...
                        <p className="text-xs text-slate-500 dark:text-neutral-400">
                            {search.length > 0
                                ? `${visibleCount} section cocok untuk pencarian "${searchQuery}"`
                                : 'Kelola pengaturan per kategori dengan panel yang lebih ringkas.'}
                        </p>
                    </div>
                    {search.length > 0 && (
                        <Button
                            variant="ghost"
                            className="rounded-xl text-slate-600 dark:text-neutral-300"
                                        ...
                                        >
                                            ...
                                            >
                                                <div className="grid gap-4 sm:grid-cols-2">
                                                    <FieldWrapper
                                                        label="Token TTL (detik)"
                                                        hint="Valid 30-600 detik"
                                                    >
                                                        <Input
                                                            type="number"
                                                            value={
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <FieldWrapper
                                                    ...
                                                    hint="Minimum 10m, maksimum 5000m"
                                                >
                                                    <Input
                                                        type="number"
                                                        value={
                                                ...
                                                <div className="flex justify-end">
                                                    <SaveButton
                                                        ...
                                                        isSaving={
                                                            geofenceForm.processing
                                                        }
                                                        hasChanges={
                                                            geofenceForm.isDirty
                                                        }
                                                        disabled={
                                                            !geofenceForm.isDirty
                                                        }
                                                    ...
                                                    >
                                                        <div className="relative mt-2">
                                                            <Input
                                                                ...
                                                                className="rounded-xl border-slate-300 pr-11 dark:border-neutral-600"
                                                            />
                                                            <button
                                                                type="button"
                                                                onClick={() =>
                                                                ...
                                                                className="absolute top-1/2 right-3 -translate-y-1/2 text-slate-400 transition-colors hover:text-slate-700 dark:hover:text-neutral-200"
                                                            >
                                                                {field.visible ? (
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="space-y-4">
                                                <div
                                                    className={`rounded-2xl border p-4 ${is2FAEnabled ? 'border-emerald-300 bg-emerald-50/70 text-emerald-700 dark:border-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-300' : 'border-amber-300 bg-amber-50/70 text-amber-700 dark:border-amber-600 dark:bg-amber-950/20 dark:text-amber-300'}`}
                                                >
                                                ...
                                                <div className="rounded-2xl border border-slate-200/70 bg-slate-50/70 p-4 dark:border-neutral-600 dark:bg-black/20">
                                                    <p className="text-xs font-semibold tracking-[0.24em] text-slate-500 uppercase dark:text-neutral-400">
                                                        Sesi Aktif
                                                    </p>
                                                ...
                                                <Link
                                                    href="/admin/profile"
                                                    className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50 dark:border-neutral-600 dark:bg-neutral-900 dark:text-neutral-200 dark:hover:bg-neutral-800"
                                                >
                                ...
                                {matchesSearch(
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                <div
                                                    className={`rounded-2xl border p-5 ${advancedForm.data.maintenance_mode ? 'border-amber-200 bg-amber-50/70 text-amber-700 dark:border-amber-600 dark:bg-amber-950/20 dark:text-amber-300' : 'border-slate-200 bg-slate-50/70 text-slate-700 dark:border-neutral-600 dark:bg-black/20 dark:text-neutral-200'}`}
                                                >
                                                    <AnimatedToggle
                                                        checked={
                                                            advancedForm.data
                                                    ...
                                                    emailTemplates.map(
                                                        (template) => (
                                                            <div
                                                                ...
                                                                className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-neutral-600 dark:bg-neutral-900"
                                                            >
                                                                    ...
                                                                    <Badge
                                                                        className={
                                                                            template.is_active
                                                                                ...
                                                                                : 'border-0 bg-slate-300 text-slate-800 dark:bg-neutral-700 dark:text-neutral-300'
                                                                        }
                                                                    >
                                                                        {template.is_active
                                                                            ? 'Aktif'
                                                                ...
                                                                template.id ? (
                                                                    <form
                                                                        ...
                                                                        className="mt-4 space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-neutral-600 dark:bg-black/20"
                                                                    >
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="space-y-4">
                                                <div className="grid gap-3 sm:grid-cols-2">
                                                    <Button
                                                        className="rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white"
                                                        onClick={() =>
                                                            setShowBackupDialog(
                                                                true,
                                                            )
                                                ...
                                                {recentHistory.length > 0 ? (
                                                    recentHistory
                                                        ...
                                                        .map((item) => (
                                                            <div
                                                                key={item.id}
                                                                className="rounded-2xl border border-slate-200/70 bg-slate-50/70 p-4 dark:border-neutral-600 dark:bg-black/20"
                                                            >
                                                                <div className="flex items-start justify-between gap-3">
                                                                    ...
                                                                    <Badge className="border-0 bg-slate-900 text-white dark:bg-white dark:text-slate-900">
                                                                        {
                                                                            item.change_type
                                                                        }
                                                                    </Badge>
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="grid gap-3 sm:grid-cols-2">
                                                <Button
                                                    variant="outline"
                                                    className="h-auto rounded-2xl px-5 py-4 text-left"
                                                    onClick={clearCache}
                                                >
                                                    <div className="flex items-start gap-3">
                ...
                {visibleCount === 0 && (
                    <motion.div
                        variants={itemVariants}
                        className="rounded-3xl border border-dashed border-slate-300 bg-white/70 px-6 py-16 text-center shadow-lg dark:border-neutral-700 dark:bg-neutral-900/70"
                    >
                        ...
                        <p className="mt-2 text-sm text-slate-500 dark:text-neutral-400">
                            Ubah kata kunci pencarian atau pindah ke kategori
                            lain.
                        </p>
                    </motion.div>
                )}
            ...
            <BackupDialog
                ...
                onBackupFieldChange={(field, value) =>
                    backupForm.setData(field, value)
                }
    ...
    return (
        <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 backdrop-blur-xl">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <p className="text-[11px] font-semibold tracking-[0.18em] text-indigo-100/80 uppercase">
                        {label}
                    </p>
...
function SectionCard({
    ...
    const toneClass = {
        ...
        emerald:
            'from-emerald-500/10 to-teal-500/10 border-emerald-200/60 dark:border-emerald-900/50',
    ...
    return (
        <div
            className={`rounded-3xl border bg-gradient-to-br ${toneClass} bg-white/80 p-6 shadow-xl backdrop-blur-xl dark:bg-neutral-900/80`}
        >
...
function InfoTile({ label, value }: { label: string; value: string }) {
    return (
        <div className="rounded-2xl border border-slate-200/70 bg-slate-50/80 p-4 dark:border-neutral-600 dark:bg-black/20">
            <p className="text-[11px] font-semibold tracking-[0.2em] text-slate-500 uppercase dark:text-neutral-400">
                {label}
            </p>
            <p className="mt-2 text-sm font-bold break-all text-slate-900 dark:text-white">
                {value}
            </p>
        </div>
    );
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Simpan snapshot pengaturan sebelum perubahan besar dan
                ...
                <div className="space-y-6">
                    <form
                        onSubmit={onSubmit}
                        className="rounded-3xl border border-violet-200/60 bg-violet-50/60 p-5 dark:border-violet-600 dark:bg-violet-950/20"
                    >
                        <div className="grid gap-4 lg:grid-cols-[1fr,1fr,auto] lg:items-end">
                            <div>
                                ...
                                <Input
                                    ...
                                    className="mt-2 rounded-xl border-slate-300 dark:border-neutral-600"
                                    placeholder="Contoh: Backup sebelum update semester baru"
                                />
                            ...
                            <div>
                                ...
                                <Textarea
                                    ...
                                    className="mt-2 rounded-xl border-slate-300 dark:border-neutral-600"
                                    rows={3}
                                    placeholder="Opsional, jelaskan konteks backup ini."
                                />
                                <InputError
                                    message={backupErrors.description}
                                />
                            </div>
                            <Button
                                type="submit"
                    ...
                    <div className="space-y-3">
                        {backups.length === 0 ? (
                            ...
                            backups.map((backup) => (
                                <div
                                    key={backup.id}
                                    className="rounded-2xl border border-slate-200/70 bg-white p-4 shadow-sm dark:border-neutral-600 dark:bg-neutral-900"
                                >
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Audit trail untuk perubahan nilai konfigurasi di halaman
                        pengaturan admin.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-3">
                    {recentHistory.length === 0 ? (
                        ...
                        recentHistory.map((item) => (
                            <div
                                key={item.id}
                                className="rounded-2xl border border-slate-200/70 bg-white p-4 dark:border-neutral-600 dark:bg-neutral-900"
                            >
                                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                    <div className="space-y-3">
                                        <div>
