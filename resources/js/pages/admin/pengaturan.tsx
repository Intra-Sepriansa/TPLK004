...
import { Settings, Shield, Bell, Server, HardDrive, Save, Trash2, Activity, CheckCircle, XCircle, Database, Zap, Lock, RefreshCw, Smartphone, Mail, Calendar, FileText, BarChart3, Palette, Moon, Sun, Monitor, KeyRound, ShieldCheck, Eye, EyeOff, History, AlertTriangle, Globe, UserCog, Fingerprint } from 'lucide-react';
    ...
    return (
        <AppLayout>
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-700 to-slate-900 p-6 text-white shadow-lg">
                    <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-white/5" />
                    <div className="absolute -bottom-10 -left-10 h-32 w-32 rounded-full bg-white/5" />
                    <div className="relative">
                        <div className="flex items-center gap-3">
                            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/10 backdrop-blur"><Settings className="h-6 w-6" /></div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-slate-900 text-white shadow-lg dark:bg-white dark:text-slate-900' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700 dark:hover:bg-slate-700'}`}>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                                ...
                                <form onSubmit={submitSecurity} className="space-y-4">
                                    <div><Label className="text-slate-700 dark:text-slate-300">Token TTL (detik)</Label><Input type="number" value={securityForm.data.token_ttl_seconds} onChange={e => securityForm.setData('token_ttl_seconds', parseInt(e.target.value))} className="mt-1 dark:bg-slate-900 dark:border-slate-700" /><p className="text-xs text-slate-500 mt-1">Durasi validitas QR code (30-600 detik)</p></div>
                                    <div><Label className="text-slate-700 dark:text-slate-300">Toleransi Terlambat (menit)</Label><Input type="number" value={securityForm.data.late_minutes} onChange={e => securityForm.setData('late_minutes', parseInt(e.target.value))} className="mt-1 dark:bg-slate-900 dark:border-slate-700" /></div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                                ...
                                <form onSubmit={submitNotifications} className="space-y-4">
                                    <div className="space-y-3">
                                        <label className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50 cursor-pointer">
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                                ...
                                <form onSubmit={submitAdvanced} className="space-y-4">
                                    <div><Label className="text-slate-700 dark:text-slate-300">Maksimal Percobaan Login</Label><Input type="number" value={advancedForm.data.max_login_attempts} onChange={e => advancedForm.setData('max_login_attempts', parseInt(e.target.value))} className="mt-1 dark:bg-slate-900 dark:border-slate-700" /><p className="text-xs text-slate-500 mt-1">Jumlah percobaan sebelum akun dikunci (3-10)</p></div>
                                    <div><Label className="text-slate-700 dark:text-slate-300">Durasi Lockout (menit)</Label><Input type="number" value={advancedForm.data.lockout_duration} onChange={e => advancedForm.setData('lockout_duration', parseInt(e.target.value))} className="mt-1 dark:bg-slate-900 dark:border-slate-700" /><p className="text-xs text-slate-500 mt-1">Durasi akun dikunci setelah gagal login (5-60)</p></div>
                                    <div><Label className="text-slate-700 dark:text-slate-300">Session Lifetime (menit)</Label><Input type="number" value={advancedForm.data.session_lifetime} onChange={e => advancedForm.setData('session_lifetime', parseInt(e.target.value))} className="mt-1 dark:bg-slate-900 dark:border-slate-700" /><p className="text-xs text-slate-500 mt-1">Durasi sesi login aktif (30-480)</p></div>
                            ...
                            <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div><Label className="text-slate-700 dark:text-slate-300">Threshold Kecocokan Wajah (%)</Label><Input type="number" value={advancedForm.data.face_match_threshold} onChange={e => advancedForm.setData('face_match_threshold', parseInt(e.target.value))} className="mt-1 dark:bg-slate-900 dark:border-slate-700" /><p className="text-xs text-slate-500 mt-1">Persentase minimum kecocokan wajah (50-99)</p></div>
                                    <label className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50 cursor-pointer">
                            ...
                            <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                                ...
                                <div className="space-y-2">
                                    {[{ label: 'PHP Version', value: systemInfo.php_version }, { label: 'Laravel Version', value: systemInfo.laravel_version }, { label: 'Server Time', value: systemInfo.server_time }, { label: 'Timezone', value: systemInfo.timezone }, { label: 'Environment', value: systemInfo.environment }, { label: 'Debug Mode', value: systemInfo.debug_mode ? 'Aktif' : 'Nonaktif' }, { label: 'Database', value: systemInfo.db_connection }, { label: 'Cache Driver', value: systemInfo.cache_driver }].map(({ label, value }) => (
                                        <div key={label} className="flex justify-between p-2 rounded-lg bg-slate-50 dark:bg-slate-900/50"><span className="text-sm text-slate-500">{label}</span><span className="text-sm font-medium text-slate-900 dark:text-white">{value}</span></div>
                                    ))}
                                </div>
                            </div>
                            <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50"><div className="flex items-center justify-between"><span className="text-sm text-slate-600 dark:text-slate-400">Ruang Tersedia</span><span className="text-lg font-bold text-slate-900 dark:text-white">{formatBytes(storageInfo.free_space)}</span></div></div>
                                </div>
                            </div>
                            <div className="lg:col-span-2 rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                            ...
                            <div className="lg:col-span-2 rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-950/70">
                                ...
                                <div className="grid gap-3 sm:grid-cols-3">
                                    {[{ name: 'Database', status: true, icon: Database }, { name: 'Cache', status: true, icon: Zap }, { name: 'Storage', status: storageInfo.used_percentage < 95, icon: HardDrive }].map(({ name, status, icon: Icon }) => (
                                        <div key={name} className="flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50">
