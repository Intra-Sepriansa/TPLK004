...
export default function AdminRekapKehadiranDetail({
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'history' | 'courses' | 'analytics' | 'timeline' | 'actions'>('overview');
    const [historyStatus, setHistoryStatus] = useState(filters.status || 'all');
    const [historyCourse, setHistoryCourse] = useState(filters.course_id || 'all');

    const [warningTitle, setWarningTitle] = useState('');
    const [warningMessage, setWarningMessage] = useState('');
    const [appreciationTitle, setAppreciationTitle] = useState('Apresiasi Kehadiran Mahasiswa');
    const [appreciationMessage, setAppreciationMessage] = useState('Terima kasih atas kehadiran dan kedisiplinan Anda. Pertahankan performa ini.');
    ...
    const hourlyData = useMemo(
        () => hourlyDistribution.labels.map((label, index) => ({ label, total: hourlyDistribution.values[index] ?? 0 })),
        [hourlyDistribution],
    );

    const courseRateData = useMemo(
        () => courseBreakdown.map((course) => ({ name: course.nama, rate: course.rate })),
        [courseBreakdown],
    );

    const filteredHistory = useMemo(() => {
        return attendanceLogs.data.filter((log) => {
            const statusPass = historyStatus === 'all' || log.status === historyStatus;
            const coursePass =
                historyCourse === 'all' ||
    ...
    const handleExportReport = () => {
        ...
        window.open(`/admin/rekap-kehadiran/pdf?${params.toString()}`, '_blank');
    };
    ...
    const sendMessage = (type: 'warning' | 'appreciation') => {
        const title = type === 'warning' ? warningTitle.trim() : appreciationTitle.trim();
        const message = type === 'warning' ? warningMessage.trim() : appreciationMessage.trim();
    ...
    const predictionDelta = Number((stats.predicted_rate - stats.attendance_rate).toFixed(1));

    return (
        <AppLayout>
            <Head title={`Detail Kehadiran - ${student.nama}`} />

            <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-6 p-4 sm:p-6">
                                ...
                                <div className="relative h-20 w-20 shrink-0 sm:h-24 sm:w-24">
                                    ...
                                    <div
                                        className={cn(
                                            ...
                                            student.status === 'at_risk' && 'bg-rose-500',
                                        )}
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-purple-100">
                                        <span className="inline-flex items-center gap-1">
                                            <IdCard className="h-4 w-4" /> {student.nim}
                                        </span>
                                        <span>•</span>
                                        <span className="inline-flex items-center gap-1">
                                            <Users className="h-4 w-4" /> {student.kelas}
                                        </span>
                                        <span>•</span>
                                        <span className="inline-flex items-center gap-1">
                    ...
                    {quickStats.map((stat) => (
                        ...
                        >
                            ...
                            <div className="relative flex items-center justify-between gap-3">
                                {stat.icon ? (
                                    <img src={stat.icon} alt={stat.label} className="h-11 w-11 shrink-0 object-contain" />
                                ) : stat.customIcon ? (
                                    <div
                                        className={cn(
                            ...
                            <p className="relative mt-1 text-xs text-slate-500 dark:text-slate-400">{stat.label}</p>
                        </motion.div>
                    ))}
                </motion.div>

                <motion.div variants={itemVariants} className="w-full overflow-hidden rounded-2xl border border-white/10 bg-neutral-100/50 p-1 backdrop-blur-md dark:bg-neutral-900/50">
                    ...
                    <motion.div variants={itemVariants} className="space-y-6">
                        <div className="grid gap-6 lg:grid-cols-3">
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 text-sm font-bold tracking-wide text-neutral-500 uppercase">Attendance Rate</h3>
                                <div className="relative mx-auto h-52 w-52">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <RadialBarChart
                                            innerRadius="65%"
                                            outerRadius="100%"
                                            data={[{ name: 'Rate', value: stats.attendance_rate }]}
                                            startAngle={90}
                                            endAngle={-270}
                                        >
                                            <PolarAngleAxis type="number" domain={[0, 100]} angleAxisId={0} tick={false} />
                                            <RadialBar dataKey="value" cornerRadius={12} fill="#6366f1" background />
                                        </RadialBarChart>
                                    </ResponsiveContainer>
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <p className="text-3xl font-black text-neutral-900 dark:text-white">{stats.attendance_rate}%</p>
                            ...
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 flex items-center gap-2 text-sm font-bold tracking-wide text-neutral-500 uppercase">
                                    <Target className="h-4 w-4" /> Perbandingan Kelas
                                </h3>
                                <div className="space-y-5">
                                    <div>
                                        <div className="mb-1 flex items-center justify-between text-sm">
                                            <span className="font-semibold text-neutral-700 dark:text-neutral-200">{student.nama}</span>
                                            <span className="font-bold text-indigo-600">{stats.attendance_rate}%</span>
                                        </div>
                                        <div className="h-3 overflow-hidden rounded-full bg-neutral-100 dark:bg-neutral-800">
                                            <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-fuchsia-500" style={{ width: `${stats.attendance_rate}%` }} />
                                        </div>
                                    </div>
                                    <div>
                                        <div className="mb-1 flex items-center justify-between text-sm">
                                            <span className="text-neutral-500">Rata-rata kelas</span>
                                            <span className="font-bold text-slate-600 dark:text-slate-300">{stats.class_avg_rate}%</span>
                                        </div>
                                        <div className="h-3 overflow-hidden rounded-full bg-neutral-100 dark:bg-neutral-800">
                                            <div className="h-full rounded-full bg-slate-400" style={{ width: `${stats.class_avg_rate}%` }} />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="rounded-xl border border-violet-200/50 bg-violet-50/70 p-3 text-center dark:border-violet-800/40 dark:bg-violet-900/10">
                                            <p className="text-lg font-bold text-violet-600">{stats.current_streak}</p>
                                        ...
                                        <div className="rounded-xl border border-cyan-200/50 bg-cyan-50/70 p-3 text-center dark:border-cyan-800/40 dark:bg-cyan-900/10">
                                            <p className="text-lg font-bold text-cyan-600">{stats.longest_streak}</p>
                                ...
                                <div className="space-y-3">
                                    {upcomingSessions.length > 0 ? (
                                        upcomingSessions.map((session) => (
                                            <div key={session.id} className="rounded-xl border border-neutral-200/70 bg-white/70 p-3 dark:border-neutral-700 dark:bg-neutral-800/60">
                                                <p className="truncate text-sm font-bold text-neutral-900 dark:text-white">{session.course}</p>
                                                <p className="text-xs text-neutral-500">Pertemuan {session.meeting_number} • {session.start_at}</p>
                                            </div>
                                        ))
                                    ) : (
                                        <p className="py-10 text-center text-sm text-neutral-500">Belum ada sesi mendatang.</p>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                            <div className="mb-5 flex items-center gap-2">
                                ...
                                <h3 className="text-lg font-bold text-neutral-900 dark:text-white">Tren Kehadiran 30 Hari Terakhir</h3>
                            </div>
                            <div className="h-80">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={trendData}>
                                        ...
                                        <Tooltip />
                                        <Legend />
                                        <Area type="monotone" dataKey="Hadir" stroke="#10b981" fill="url(#gradPresent)" strokeWidth={2} />
                                        <Area type="monotone" dataKey="Terlambat" stroke="#f59e0b" fill="url(#gradLate)" strokeWidth={2} />
                                        <Area type="monotone" dataKey="Ditolak" stroke="#ef4444" fill="url(#gradRejected)" strokeWidth={2} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                ...
                {activeTab === 'history' && (
                    <motion.div variants={itemVariants} className="space-y-4">
                        <div className="grid gap-3 rounded-2xl border border-white/20 bg-white/40 p-4 shadow-lg backdrop-blur-xl sm:grid-cols-4 dark:border-neutral-800 dark:bg-neutral-900/40">
                            <div>
                                <label className="mb-1 block text-xs font-bold tracking-wide text-neutral-500 uppercase">Status</label>
                                <select
                                    value={historyStatus}
                            ...
                            <div>
                                <label className="mb-1 block text-xs font-bold tracking-wide text-neutral-500 uppercase">Mata Kuliah</label>
                                <select
                                    value={historyCourse}
                        ...
                        <div className="space-y-3">
                            {filteredHistory.length > 0 ? (
                                filteredHistory.map((log) => (
                                    <div key={log.id} className="rounded-2xl border border-white/20 bg-white/40 p-4 shadow-lg backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                            <div className="min-w-0">
                                                <p className="truncate text-sm font-bold text-neutral-900 dark:text-white">{log.session?.course?.nama ?? '-'}</p>
                                                <p className="text-xs text-neutral-500">
                                                    Pertemuan {log.session?.meeting_number ?? '-'} • {new Date(log.scanned_at).toLocaleString('id-ID')}
                                                </p>
                                            </div>
                                        ...
                                        <div className="mt-3 flex flex-wrap gap-3 border-t border-neutral-200/60 pt-3 text-xs text-neutral-500 dark:border-neutral-700/70">
                                            {log.device_model && (
                                                <span className="inline-flex items-center gap-1">
                                                    <Smartphone className="h-3.5 w-3.5" /> {log.device_model}
                                                </span>
                                            )}
                                            {log.device_os && (
                                                <span className="inline-flex items-center gap-1">
                                                    <Clock className="h-3.5 w-3.5" /> {log.device_os}
                                                </span>
                                            )}
                                            {typeof log.distance_m === 'number' && (
                                                <span className="inline-flex items-center gap-1">
                                                    <MapPin className="h-3.5 w-3.5" /> {log.distance_m} m
                                                </span>
                                            )}
                                            {log.selfie_verification && (
                                                <span className="inline-flex items-center gap-1">
                                                    <Eye className="h-3.5 w-3.5" /> Selfie: {log.selfie_verification.status}
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="rounded-2xl border border-white/20 bg-white/40 p-10 text-center text-sm text-neutral-500 backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                    Tidak ada data kehadiran dengan filter saat ini.
                                </div>
                            )}
                        </div>
                ...
                {activeTab === 'courses' && (
                    <motion.div variants={itemVariants} className="grid gap-6 lg:grid-cols-2">
                        <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                            <h3 className="mb-5 text-lg font-bold text-neutral-900 dark:text-white">Perbandingan Rate per Mata Kuliah</h3>
                            <div className="h-80">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={courseRateData}>
                                        ...
                                        <Bar dataKey="rate" radius={[10, 10, 0, 0]}>
                                            {courseRateData.map((entry, idx) => (
                                                <Cell key={`course-cell-${idx}`} fill={entry.rate >= 80 ? '#10b981' : entry.rate >= 60 ? '#f59e0b' : '#ef4444'} />
                                            ))}
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
                        ...
                        <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                            <h3 className="mb-5 text-lg font-bold text-neutral-900 dark:text-white">Ringkasan Mata Kuliah</h3>
                            <div className="space-y-3">
                                {courseBreakdown.length > 0 ? (
                                    courseBreakdown.map((course) => (
                                        <div key={course.id} className="rounded-xl border border-neutral-200/70 bg-white/70 p-3 dark:border-neutral-700 dark:bg-neutral-800/60">
                                            ...
                                            <div className="mt-2 grid grid-cols-4 gap-2 text-center">
                                                <div>
                                                    <p className="text-sm font-bold text-indigo-600">{course.total_sessions}</p>
                                                    <p className="text-[10px] text-neutral-500">Sesi</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-emerald-600">{course.present}</p>
                                                    <p className="text-[10px] text-neutral-500">Hadir</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-amber-600">{course.late}</p>
                                                    <p className="text-[10px] text-neutral-500">Late</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-violet-600">{course.rate}%</p>
                                    ...
                                    <p className="py-12 text-center text-sm text-neutral-500">Belum ada data mata kuliah.</p>
                                )}
                            </div>
                        </div>
                    </motion.div>
                ...
                {activeTab === 'analytics' && (
                    <motion.div variants={itemVariants} className="space-y-6">
                        <div className="grid gap-6 lg:grid-cols-2">
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">Distribusi Jam Kehadiran</h3>
                                <div className="h-72">
                                    <ResponsiveContainer width="100%" height="100%">
                            ...
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">Pola Hari Kehadiran</h3>
                                <div className="h-72">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={dayOfWeekData}>
                                            ...
                                            <Bar dataKey="count" radius={[8, 8, 0, 0]}>
                                                {dayOfWeekData.map((_, idx) => (
                                                    <Cell key={`day-cell-${idx}`} fill={idx < 5 ? '#10b981' : '#94a3b8'} />
                                                ))}
                                            </Bar>
                                        </BarChart>
                                    </ResponsiveContainer>
                        ...
                        <div className="grid gap-6 lg:grid-cols-3">
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl lg:col-span-2 dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">Ringkasan Bulanan</h3>
                                <div className="h-72">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={monthlySummary}>
                                            ...
                                            <Area type="monotone" dataKey="late" stroke="#f59e0b" fill="#f59e0b33" strokeWidth={2} />
                                            <Area type="monotone" dataKey="rejected" stroke="#ef4444" fill="#ef444433" strokeWidth={2} />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
                            ...
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 text-sm font-bold tracking-wide text-neutral-500 uppercase">Prediksi Kehadiran</h3>
                                <div className="space-y-4">
                                    <div className="rounded-2xl border border-indigo-200/50 bg-indigo-50/70 p-4 dark:border-indigo-800/40 dark:bg-indigo-900/10">
                                        <p className="text-xs text-neutral-500">Rate Saat Ini</p>
                                        <p className="text-3xl font-black text-indigo-600">{stats.attendance_rate}%</p>
                                    </div>
                                    <div className="rounded-2xl border border-fuchsia-200/50 bg-fuchsia-50/70 p-4 dark:border-fuchsia-800/40 dark:bg-fuchsia-900/10">
                                        <p className="text-xs text-neutral-500">Prediksi Akhir</p>
                                        <p className="text-3xl font-black text-fuchsia-600">{stats.predicted_rate}%</p>
                                    </div>
                                    <div className={cn(
                                        ...
                                        {predictionDelta >= 0 ? '+' : ''}{predictionDelta}% dibanding rate saat ini
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <motion.div variants={itemVariants} className="space-y-6">
                        <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                            <div className="mb-4 flex items-center justify-between gap-3">
                                <h3 className="text-lg font-bold text-neutral-900 dark:text-white">Calendar Heatmap</h3>
                                <div className="flex items-center gap-3 text-[11px] text-neutral-500">
                                    <span className="inline-flex items-center gap-1"><span className="h-3 w-3 rounded bg-neutral-200 dark:bg-neutral-700" />None</span>
                                    <span className="inline-flex items-center gap-1"><span className="h-3 w-3 rounded bg-emerald-300/70" />Low</span>
                                    <span className="inline-flex items-center gap-1"><span className="h-3 w-3 rounded bg-emerald-400/80" />Medium</span>
                                    <span className="inline-flex items-center gap-1"><span className="h-3 w-3 rounded bg-emerald-500" />High</span>
                                </div>
                            </div>
                                ...
                                {heatmapEntries.length > 0 ? (
                                    heatmapEntries.map(([date, level]) => (
                                        <div
                                            key={date}
                                            title={`${date} • ${level}`}
                                            className={cn('h-4 w-full rounded-sm transition-transform hover:scale-125', heatmapClass(level))}
                                        />
                                    ))
                                ) : (
                                    <p className="col-span-14 py-8 text-center text-sm text-neutral-500">Belum ada data heatmap.</p>
                                )}
                            </div>
                        </div>

                        <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                            <h3 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">Timeline Aktivitas Kehadiran</h3>
                            <div className="space-y-3">
                                {timeline.length > 0 ? (
                                    timeline.map((item) => (
                                        <div key={item.id} className="rounded-xl border border-neutral-200/70 bg-white/70 p-3 dark:border-neutral-700 dark:bg-neutral-800/60">
                                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                <div>
                                                    <p className="text-sm font-bold text-neutral-900 dark:text-white">{item.course}</p>
                                                    <p className="text-xs text-neutral-500">Pertemuan {item.meeting_number ?? '-'} • {item.time}</p>
                                                </div>
                                                {getAttendanceStatusBadge(item.status)}
                                            </div>
                                            <div className="mt-2 flex flex-wrap gap-3 text-xs text-neutral-500">
                                                {item.distance_m != null && (
                                                    <span className="inline-flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> {item.distance_m} m</span>
                                                )}
                                                {item.device && (
                                                    <span className="inline-flex items-center gap-1"><Smartphone className="h-3.5 w-3.5" /> {item.device}</span>
                                                )}
                                                {item.selfie_status && (
                                                    <span className="inline-flex items-center gap-1"><Eye className="h-3.5 w-3.5" /> Selfie: {item.selfie_status}</span>
                                                )}
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <p className="py-8 text-center text-sm text-neutral-500">Belum ada timeline aktivitas.</p>
                                )}
                            </div>
                        </div>
                    </motion.div>
                ...
                {activeTab === 'actions' && (
                    <motion.div variants={itemVariants} className="space-y-6">
                        <div className="grid gap-6 lg:grid-cols-2">
                            <div className="rounded-3xl border border-amber-200/50 bg-amber-50/60 p-6 shadow-xl backdrop-blur-xl dark:border-amber-800/30 dark:bg-amber-900/10">
                                <h3 className="mb-4 flex items-center gap-2 text-lg font-bold text-amber-700 dark:text-amber-300">
                                    <MessageSquareWarning className="h-5 w-5" /> Kirim Warning
                                </h3>
                                <div className="space-y-3">
                                    <input
                                        value={warningTitle}
                                    ...
                                    >
                                        <Send className="h-4 w-4" /> Kirim Warning
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="rounded-3xl border border-emerald-200/50 bg-emerald-50/60 p-6 shadow-xl backdrop-blur-xl dark:border-emerald-800/30 dark:bg-emerald-900/10">
                                <h3 className="mb-4 flex items-center gap-2 text-lg font-bold text-emerald-700 dark:text-emerald-300">
                                    <Sparkles className="h-5 w-5" /> Kirim Apresiasi
                                </h3>
                                <div className="space-y-3">
                                    <input
                                        value={appreciationTitle}
                                        onChange={(e) => setAppreciationTitle(e.target.value)}
                                    ...
                                    />
                                    <button
                                        onClick={() => sendMessage('appreciation')}
                                    ...
                                    >
                                        <Award className="h-4 w-4" /> Kirim Apresiasi
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">Kontak Mahasiswa</h3>
                            ...
                            <div className="rounded-3xl border border-white/20 bg-white/40 p-6 shadow-xl backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/40">
                                <h3 className="mb-4 text-lg font-bold text-neutral-900 dark:text-white">Riwayat Warning & Apresiasi</h3>
                                <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
                                    {warnings.length > 0 ? (
                                        warnings.map((warning) => (
                                            <div key={warning.id} className="rounded-xl border border-neutral-200/70 bg-white/70 p-3 dark:border-neutral-700 dark:bg-neutral-800/60">
                                                <div className="mb-1 flex items-center justify-between gap-2">
                                                    ...
                                                    <span className="text-[11px] text-neutral-500">{warning.created_at}</span>
                                                </div>
                                                <p className="text-sm font-bold text-neutral-900 dark:text-white">{warning.title}</p>
                                                <p className="mt-1 line-clamp-2 text-xs text-neutral-500">{warning.message}</p>
                                            </div>
                                        ))
                                    ) : (
                                        <p className="py-8 text-center text-sm text-neutral-500">Belum ada riwayat warning/apresiasi.</p>
                                    )}
                                </div>
                            </div>
                        </div>
