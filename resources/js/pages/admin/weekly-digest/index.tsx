...
const statCards = [
    ...
    },
    {
        ...
        imageSrc: DraftIcon,
        gradientBg:
            'from-slate-200/50 to-white/50 dark:from-slate-800/50 dark:to-neutral-900/50',
        glow: 'bg-slate-300',
    },
    {
        key: 'current_week',
...
export default function WeeklyDigestIndex({
    digests,
    semesters,
    weeks,
    stats,
    ...
    const [week, setWeek] = useState(
        filters.week ? String(filters.week) : 'all',
    );
    ...
    const handleBatchExport = () => {
        ...
        setIsExporting(true);
        router.post(
            '/admin/weekly-digest/batch-export',
            { digest_ids: selectedIds },
            {
                preserveScroll: true,
                preserveState: true,
            },
    ...
    return (
        <AppLayout>
            ...
            >
                            ...
                            <div className="flex flex-col items-center gap-5 text-center sm:flex-row sm:items-start sm:text-left">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="mt-2 max-w-2xl text-sm text-indigo-100/90">
                                        Gabungkan entry menjadi rekap pekanan
                                        mahasiswa secara otomatis.
                                    </p>
                                </div>
                            </div>

                            <div className="mt-4 flex w-full flex-col items-center gap-3 sm:mt-0 sm:items-start lg:w-auto lg:items-end">
                    ...
                    {statCards.map((card) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className="group relative overflow-hidden rounded-2xl border border-white/20 bg-white/40 p-4 shadow-xl backdrop-blur-xl transition-all sm:rounded-3xl sm:p-6 dark:border-white/5 dark:bg-neutral-900/40"
                            >
                                <div
                                    className={cn(
                                ...
                                <div
                                    className={cn(
                                        'absolute -top-8 -right-8 h-28 w-28 rounded-full opacity-15 blur-3xl transition-all group-hover:opacity-30',
                                        card.glow,
                                    )}
                                />
                ...
                <motion.div
                    variants={itemVariants}
                    className="rounded-3xl border border-white/20 bg-white/50 p-5 shadow-lg backdrop-blur-xl dark:border-white/5 dark:bg-neutral-900/50"
                >
                    <div className="mb-5 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                                Pilah daftar matkul yang sudah masuk ke Mentari
                                pada pekan aktif.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid gap-3 lg:grid-cols-[1.5fr_1fr_0.9fr_0.9fr_auto] lg:items-center">
                        ...
                        <Select value={semester} onValueChange={setSemester}>
                            <SelectTrigger className="h-11 rounded-2xl border-white/20 bg-white/80 dark:border-white/10 dark:bg-neutral-800/80">
                ...
                <motion.div variants={itemVariants} className="space-y-4">
                    {digests.data.length > 0 && (
                        <motion.div
                            variants={itemVariants}
                            className="flex items-center justify-between rounded-2xl bg-white/50 p-4 shadow-md backdrop-blur-xl dark:bg-neutral-900/50"
                        >
                    ...
                    {digests.data.length === 0 ? (
                        <div className="rounded-3xl border border-dashed border-slate-300 bg-white/60 p-14 text-center shadow-lg backdrop-blur-xl dark:border-white/10 dark:bg-neutral-900/50">
                            ...
                            <p className="text-lg font-semibold text-slate-900 dark:text-white">
                                Belum ada entry pekanan
                            </p>
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                Tambahkan matkul yang materinya sudah masuk pada
                                pekan ini.
                            </p>
                        </div>
                    ) : (
                        digests.data.map((digest, index) => (
                            <motion.div
                                ...
                                className={cn(
                                    'group relative cursor-pointer overflow-hidden rounded-3xl border p-5 shadow-xl backdrop-blur-xl transition-colors hover:border-indigo-500/30',
                                    digest.is_published
                            ...
                            >
                                ...
                                <div
                                    className={cn(
                                        'absolute -top-12 -right-12 h-32 w-32 rounded-full blur-3xl transition-all',
                                        digest.is_published
                                            ? 'bg-emerald-500/20 group-hover:bg-emerald-500/30'
                                            : 'bg-slate-500/15 group-hover:bg-slate-500/25',
                                    )}
                                />
                                ...
                                <div className="relative z-10 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                                    ...
                                    <div className="flex flex-wrap gap-2 xl:w-auto">
                                        <Button
                                            type="button"
                                            variant="outline"
                                            className="rounded-2xl border-white/20 bg-white/70 dark:border-white/10 dark:bg-neutral-900/60"
                                            onClick={() =>
                                                router.get(
                ...
                <ConfirmDialog
                    ...
                    onConfirm={() => {
                        if (deleteDialog.id) {
                            router.delete(
                                `/admin/weekly-digest/${deleteDialog.id}`,
                                {
                                    preserveScroll: true,
                                    onSuccess: () =>
                                        setDeleteDialog({
