...
import { ArrowLeft, BookOpen, Download, Eye, EyeOff, Globe, Trash2, Lightbulb } from 'lucide-react';
          ...
          <div className="relative space-y-5">
            ...
            <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
              <div className="min-w-0">
                <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-start sm:text-left">
                  ...
                  <div className="min-w-0">
                    ...
                    <h1 className="mt-1 font-bold text-2xl sm:text-3xl leading-tight">{digest.display_title}</h1>
                  </div>
                </div>

                <div className="mt-5 flex flex-wrap justify-center gap-2 sm:justify-start">
                  {digest.courses.map((course) => (
                    <Badge key={course.id} className="border-indigo-300/30 bg-indigo-500/20 px-3 py-1.5 text-indigo-100 backdrop-blur-md">
              ...
              <div className="flex flex-wrap justify-center gap-3 xl:max-w-[360px] xl:justify-end">
                <Button type="button" onClick={() => router.get(`/admin/weekly-digest/${digest.id}/edit`)} className="h-11 rounded-xl bg-white/20 text-white backdrop-blur-md hover:bg-white/30">
                  <BookOpen className="mr-2 h-4 w-4" />
                  Edit
                </Button>
                <Button type="button" onClick={() => router.patch(`/admin/weekly-digest/${digest.id}/publish`, {}, { preserveScroll: true })} className={cn('h-11 rounded-xl text-white shadow-lg', digest.is_published ? 'bg-amber-500 hover:bg-amber-600' : 'bg-emerald-500 hover:bg-emerald-600')}>
                  {digest.is_published ? <EyeOff className="mr-2 h-4 w-4" /> : <Eye className="mr-2 h-4 w-4" />}
                  {digest.is_published ? 'Unpublish' : 'Publish'}
                </Button>
                <Button type="button" onClick={() => window.open(`/admin/weekly-digest/${digest.id}/export-pdf`, '_blank', 'noopener,noreferrer')} className="h-11 rounded-xl bg-white/20 text-white backdrop-blur-md hover:bg-white/30">
                  <Download className="mr-2 h-4 w-4" />
                  Export PDF
                </Button>
                <Button type="button" onClick={() => setDeleteDialog(true)} className="h-11 rounded-xl bg-rose-500 text-white shadow-lg hover:bg-rose-600">
          ...
          <motion.div variants={itemVariants} className="h-full">
            <div className="h-full rounded-3xl border border-white/20 bg-white/50 p-6 shadow-lg backdrop-blur-xl dark:border-white/5 dark:bg-neutral-900/50">
              ...
              <div className="space-y-3 text-sm">
                <InfoRow label="Mata Kuliah" value={digest.courses.map(c => c.name).join(', ') || '-'} />
                <InfoRow label="Dosen" value={digest.courses.map(c => c.dosen_name).filter(Boolean).join(', ') || '-'} />
                ...
                <InfoRow label="Pertemuan" value={digest.courses.length > 1 ? 'Multi Pertemuan' : `Pertemuan ${digest.courses[0].meeting_number}`} />
          ...
          <motion.div variants={itemVariants} className="space-y-6">
            <div className="rounded-3xl border border-white/20 bg-white/50 p-6 shadow-lg backdrop-blur-xl dark:border-white/5 dark:bg-neutral-900/50">
              ...
              <div className="space-y-4 rounded-3xl border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-5 text-white shadow-2xl">
                <div>
                  ...
                  <p className="mt-2 text-2xl font-bold">{digest.display_title}</p>
                  <p className="mt-1 text-sm text-slate-300">{digest.courses.length > 1 ? `${digest.courses.length} Mata Kuliah Terpilih` : digest.courses[0].name}</p>
                </div>

                <div className="flex flex-wrap gap-2">
                  <Badge className="border-white/10 bg-white/10 text-white">
                    {digest.courses.length > 1 ? 'Multi Pertemuan' : `Pertemuan ${digest.courses[0].meeting_number}`}
                  </Badge>
            ...
            <div className="flex flex-1 flex-col rounded-3xl border border-white/20 bg-white/50 p-6 shadow-lg backdrop-blur-xl dark:border-white/5 dark:bg-neutral-900/50">
              <div className="mb-5 flex shrink-0 items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white">Tips & Panduan</h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Rekomendasi pengelolaan entry ke platform Mentari.</p>
                </div>
              </div>
              ...
              <div className="flex-1 space-y-4 rounded-b-2xl">
                <div className="rounded-2xl border border-amber-200/60 bg-amber-50/60 p-4 dark:border-amber-900/30 dark:bg-amber-900/10">
                  <p className="text-sm font-bold text-amber-900 dark:text-amber-200">Periksa Tautan Tujuan</p>
                  <p className="mt-1.5 text-xs font-medium leading-relaxed text-amber-700/90 dark:text-amber-400/80">
                    Pastikan tautan forum diskusi sudah sesuai dengan sesi pekan berjalan agar mahasiswa diarahkan ke topik diskusi yang tepat.
                  </p>
                </div>
                <div className="rounded-2xl border border-blue-200/60 bg-blue-50/60 p-4 dark:border-blue-900/30 dark:bg-blue-900/10">
                  <p className="text-sm font-bold text-blue-900 dark:text-blue-200">Manajemen Status Draft</p>
                  <p className="mt-1.5 text-xs font-medium leading-relaxed text-blue-700/90 dark:text-blue-400/80">
                    Entry dalam status Draft belum bisa dilihat oleh mahasiswa. Pastikan Anda melakukan "Publish" jika materi sudah siap untuk dikerjakan.
                  </p>
                </div>
                <div className="rounded-2xl border border-emerald-200/60 bg-emerald-50/60 p-4 dark:border-emerald-900/30 dark:bg-emerald-900/10">
                  <p className="text-sm font-bold text-emerald-900 dark:text-emerald-200">Evaluasi Kehadiran</p>
                  <p className="mt-1.5 text-xs font-medium leading-relaxed text-emerald-700/90 dark:text-emerald-400/80">
                    Mengecek secara berkala keaktifan diskusi mahasiswa sesuai dengan batas waktu (deadline) sesi pertemuan sebelum merekap data.
                  </p>
                </div>
              </div>
            </div>
