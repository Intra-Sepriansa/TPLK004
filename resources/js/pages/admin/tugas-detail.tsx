...
export default function AdminTugasDetail({ tugas, diskusi }: Props) {
    ...
    const togglePin = (id: number) => {
        router.patch(`/admin/tugas/diskusi/${id}/pin`, {}, { preserveScroll: true });
    };

    const deleteMessage = (id: number) => {
        if (confirm('Hapus pesan ini?')) {
            router.delete(`/admin/tugas/diskusi/${id}`, { preserveScroll: true });
        }
    };
    ...
    return (
        <AppLayout>
            ...
            <div className="p-6 space-y-6">
                <Button variant="ghost" onClick={() => router.visit('/admin/tugas')} className="mb-4">
                        ...
                        <div className="rounded-xl border bg-card p-4">
                            ...
                            <div className="space-y-3 text-sm">
                                ...
                                <div className="flex justify-between"><span className="text-muted-foreground">Sisa Waktu</span>
                                    <span className={tugas.is_overdue ? 'text-red-600' : ''}>
                            ...
                            diskusi.map((d) => (
                                <div key={d.id} className={`flex gap-3 ${d.is_pinned ? 'bg-yellow-50 dark:bg-yellow-900/10 p-3 rounded-lg border border-yellow-200' : ''}`}>
                                    <Avatar className="h-8 w-8">
                                        <AvatarFallback className={getSenderColor(d.sender_type)}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex gap-2 mt-2">
                                            <Button variant="ghost" size="sm" onClick={() => togglePin(d.id)}>
                                                <Pin className="h-3 w-3 mr-1" /> {d.is_pinned ? 'Unpin' : 'Pin'}
                                            </Button>
                                            <Button variant="ghost" size="sm" onClick={() => deleteMessage(d.id)} className="text-red-600">
