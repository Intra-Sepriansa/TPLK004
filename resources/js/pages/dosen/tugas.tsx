...
export default function DosenTugas({ tugasList, courses, stats, filters }: Props) {
    ...
    const handleDelete = (id: number) => {
        if (confirm('Yakin ingin menghapus tugas ini?')) router.delete(`/dosen/tugas/${id}`);
    };
    ...
    const getStatusBadge = (s: string) => {
        const c: Record<string, string> = { published: 'bg-emerald-100 text-emerald-700', draft: 'bg-gray-100 text-gray-700', closed: 'bg-red-100 text-red-700' };
        return <Badge className={c[s]}>{s}</Badge>;
    };
        ...
        <DosenLayout>
            ...
            <div className="space-y-6 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-indigo-700 dark:text-indigo-400">Informasi Tugas</h1>
                        <p className="text-muted-foreground">Kelola tugas untuk mahasiswa</p>
                    </div>
                    <Dialog open={showCreate} onOpenChange={setShowCreate}>
                        <DialogTrigger asChild>
                            <Button className="bg-indigo-600 hover:bg-indigo-700"><Plus className="mr-2 h-4 w-4" /> Tambah Tugas</Button>
                        </DialogTrigger>
                        <DialogContent className="max-w-lg">
                            ...
                            <div className="space-y-4">
                                ...
                                <Button onClick={handleCreate} className="w-full bg-indigo-600 hover:bg-indigo-700">Simpan</Button>
                            </div>
                        </DialogContent>
                    </Dialog>
                </div>
                ...
                <div className="grid grid-cols-4 gap-4">
                    ...
                    <div className="rounded-xl border bg-card p-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div><p className="text-2xl font-bold">{stats.published}</p><p className="text-sm text-muted-foreground">Published</p></div>
                        </div>
                    </div>
                    <div className="rounded-xl border bg-card p-4">
                        <div className="flex items-center gap-3">
                            <div className="rounded-lg bg-gray-100 p-2"><Clock className="h-5 w-5 text-gray-600" /></div>
                            <div><p className="text-2xl font-bold">{stats.draft}</p><p className="text-sm text-muted-foreground">Draft</p></div>
                        </div>
                    </div>
                    <div className="rounded-xl border bg-card p-4">
                        <div className="flex items-center gap-3">
                ...
                <div className="space-y-4">
                    {tugasList.length === 0 ? (
                        ...
                        tugasList.map((tugas) => (
                            <div key={tugas.id} className={`rounded-xl border bg-card p-4 transition-all hover:shadow-md ${tugas.is_overdue ? 'border-red-200' : ''}`}>
                                <div className="flex items-start justify-between">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            <DropdownMenuItem onClick={() => router.visit(`/dosen/tugas/${tugas.id}`)}><Eye className="mr-2 h-4 w-4" /> Lihat Detail</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => openEdit(tugas)}><Pencil className="mr-2 h-4 w-4" /> Edit</DropdownMenuItem>
                ...
                <Dialog open={showEdit} onOpenChange={setShowEdit}>
                    <DialogContent className="max-w-lg">
                        ...
                        <div className="space-y-4">
                            ...
                            <Button onClick={handleEdit} className="w-full bg-indigo-600 hover:bg-indigo-700">Simpan Perubahan</Button>
                        </div>
                    </DialogContent>
                </Dialog>
            </div>
