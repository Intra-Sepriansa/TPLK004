...
export default function DosenTugas({
    ...
    const [sortBy, setSortBy] = useState<
        'deadline' | 'priority' | 'submissions' | 'score'
    >('deadline');
    ...
    const filteredList = useMemo(() => {
        ...
        list.sort((a, b) => {
            ...
            if (sortBy === 'priority') {
                ...
                return (o[b.prioritas] || 0) - (o[a.prioritas] || 0);
            }
    ...
    const handleArchive = (id: number) => {
        router.patch(`/dosen/tugas/${id}`, {
            status: 'closed',
            judul: tugasList.find((t) => t.id === id)?.judul,
            deskripsi: tugasList.find((t) => t.id === id)?.deskripsi,
            jenis: tugasList.find((t) => t.id === id)?.jenis,
            deadline: tugasList.find((t) => t.id === id)?.deadline,
            prioritas: tugasList.find((t) => t.id === id)?.prioritas,
        });
    };
    const handleExport = (t: Tugas) => {
        const d =
            'data:application/json;charset=utf-8,' +
            encodeURIComponent(JSON.stringify(t, null, 2));
        const a = document.createElement('a');
        a.href = d;
        a.download = `tugas-${t.id}.json`;
        a.click();
    };
    ...
    const navigateDetail = (t: Tugas) => {
        router.visit(`/dosen/tugas/${t.id}`);
    };
    const openQuickPreview = (t: Tugas) => {
        setDetailTugas(t);
    ...
    const getPriorityBadge = (p: string) => {
        const s: Record<string, { bg: string; icon: any }> = {
            tinggi: { bg: 'from-red-500 to-rose-500', icon: Zap },
            sedang: { bg: 'from-amber-500 to-orange-500', icon: Target },
            rendah: { bg: 'from-emerald-500 to-green-500', icon: CheckCircle },
        };
        const st = s[p] || s.sedang;
        const Icon = st.icon;
        return (
            <span
                className={cn(
    ...
    const getStatusBadge = (s: string) => {
        const st: Record<string, { bg: string; icon: any }> = {
            published: {
                bg: 'from-emerald-500 to-teal-500',
                icon: CheckCircle,
            },
            draft: { bg: 'from-gray-400 to-gray-500', icon: FileText },
            closed: { bg: 'from-red-500 to-pink-500', icon: X },
        };
        const style = st[s] || st.draft;
    ...
    const getJenisBadge = (j: string) => {
        ...
        const st = s[j] || s.lainnya;
        const Icon = st.icon;
        return (
            <span
                className={cn(
                    'inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r px-2.5 py-1 text-xs font-medium text-white',
                    st.bg,
                )}
            >
    ...
    const summaryCards = [
        {
            ...
            label: 'Total Tugas',
            value: stats.total,
            sub: `${stats.published} aktif, ${stats.draft} draft`,
    ...
    const tabs = [
        { key: 'all', label: 'Semua', count: tugasList.length },
        {
            key: 'published',
            label: 'Published',
            count: tugasList.filter((t) => t.status === 'published').length,
        },
        {
            key: 'draft',
            label: 'Draft',
            count: tugasList.filter((t) => t.status === 'draft').length,
        },
        {
            key: 'overdue',
            label: 'Overdue',
            count: tugasList.filter((t) => t.is_overdue).length,
        },
    ];
                            ...
                            <div className="mt-2 flex w-full flex-col items-center gap-3 lg:mt-0 lg:w-auto lg:items-end">
                                ...
                                >
                                    <motion.button
                                        ...
                                        whileTap={{ scale: 0.98 }}
                                        onClick={() =>
                                            router.visit('/dosen/tugas/create')
                                        }
                ...
                >
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                            <div className="rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 p-2 text-white">
                        ...
                        <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-neutral-100/50 p-1 backdrop-blur-md dark:bg-neutral-900/50">
                            {(['list', 'grid', 'kanban'] as const).map(
                                (mode) => {
                                    const icons = {
                                        list: List,
                                        grid: Grid3x3,
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        <Select
                            ...
                            onValueChange={(v) => {
                                setJenisFilter(v);
                                applyFilters({ jenis: v });
                            }}
                        >
                            <SelectTrigger className="w-36 border-white/30 bg-white/60 dark:bg-neutral-800/60">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                        {filteredList.length} tugas ditemukan
                                    </p>
                                </div>
                            </div>
                            {selectedTugas.length > 0 && (
                                <span className="text-sm font-medium text-indigo-600">
                                    {selectedTugas.length} dipilih
                                </span>
                            )}
                        </div>
                    </div>
                        ...
                        {filteredList.length === 0 ? (
                            ...
                            >
                                <motion.div
                                    ...
                                    className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-500"
                                >
                                ...
                                <p className="mt-2 text-sm text-slate-500">
                                    Klik "Tambah Tugas" untuk memulai
                                </p>
                                <Button
                                    onClick={() =>
                                        router.visit('/dosen/tugas/create')
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                                {(['draft', 'published', 'closed'] as const)
                                    .concat(['overdue'] as any)
                                    .map((col) => {
                                        ...
                                        return (
                                            <div
                                                key={col}
                                                className="rounded-2xl border border-slate-200/50 bg-slate-50/50 p-3 dark:border-slate-700/50 dark:bg-neutral-800/30"
                                            >
                                                ...
                                                <div className="min-h-[100px] space-y-2">
                                                    {cfg.items.map((t) => (
                                                        <motion.div
                                                            ...
                                                            className="cursor-pointer rounded-xl border border-slate-200/50 bg-white p-3 shadow-sm transition-shadow hover:shadow-md dark:border-slate-700/50 dark:bg-neutral-800"
                                                        >
                                                            ...
                                                            <h4 className="line-clamp-1 text-sm font-medium text-slate-900 dark:text-white">
                                                                {t.judul}
                                                            </h4>
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                                {filteredList.map((t, i) => (
                                    <motion.div
                                        ...
                                        className="relative cursor-pointer overflow-hidden rounded-2xl border border-slate-200/50 bg-white shadow-sm transition-all hover:shadow-lg dark:border-slate-700/50 dark:bg-neutral-800/50"
                                    >
                                        <div
                                            className={cn(
                                        ...
                                        <div className="p-4">
                                            <div className="mb-3 flex flex-wrap items-center gap-2">
                                                {getJenisBadge(t.jenis)}
                                                {getPriorityBadge(t.prioritas)}
                                    ...
                                    >
                                        <div className="flex items-start justify-between">
                                            ...
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={selectedTugas.includes(
                                                        t.id,
                                                    )}
                                                    onChange={(e) => {
                                                        e.stopPropagation();
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <div className="mt-3 flex flex-wrap items-center gap-3">
                                                        ...
                                                        <span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-2.5 py-1 text-xs text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">
                                                        ...
                                                        {t.late_submissions >
                                                            0 && (
                                                            <span className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-2.5 py-1 text-xs text-red-700 dark:bg-red-900/30 dark:text-red-300">
                ...
                {showEdit && editTugas && (
                    ...
                    >
                        ...
                        <motion.div
                            ...
                            className="relative mx-4 w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-neutral-900"
                        >
                            ...
                            <div className="flex gap-3 border-t border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-neutral-800/50">
                                <Button
                                    variant="outline"
                                    onClick={() => setShowEdit(false)}
                                    className="flex-1"
            ...
            <AnimatePresence>
                {showDetail && detailTugas && (
                    ...
                    >
                        ...
                        <motion.div
                            variants={modalVariants}
                            className="relative flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-neutral-900"
                        >
                            {/* Modal Header */}
                            <div className="relative flex-shrink-0 overflow-hidden p-6 text-white">
                                <motion.div
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="mt-4 flex flex-wrap items-center gap-3">
                                        <span className="rounded-full bg-white/20 px-3 py-1 text-xs backdrop-blur">
                                            {detailTugas.submission_count}/
                                            {detailTugas.total_students}{' '}
                                            submissions
                                        </span>
                                        <span className="rounded-full bg-white/20 px-3 py-1 text-xs backdrop-blur">
                                            {detailTugas.completion_rate}%
                                            selesai
                                        </span>
                                        <span className="rounded-full bg-white/20 px-3 py-1 text-xs backdrop-blur">
                                            Avg: {detailTugas.average_score}
                                        </span>
                                        {detailTugas.is_overdue && (
                                            <span className="rounded-full bg-red-500/40 px-3 py-1 text-xs backdrop-blur">
                                                Overdue
                                            </span>
                                        )}
                                    </div>
                            ...
                            <div className="flex-shrink-0 px-6 pt-4">
                                <div className="flex w-fit gap-1 rounded-2xl border border-white/10 bg-neutral-100/50 p-1 dark:bg-neutral-800/50">
                                    {[
                                        {
                                            key: 'overview',
                                            label: 'Overview',
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                'relative rounded-xl px-3 py-1.5 text-xs font-medium transition-colors',
                                                detailTab === tab.key
                                                    ? 'text-slate-900 dark:text-white'
                                                    : 'text-slate-500',
                                            )}
                                        >
                                            {detailTab === tab.key && (
                                                <motion.div
                                                    ...
                                                    className="absolute inset-0 rounded-xl bg-white shadow-sm dark:bg-neutral-700"
                                                    transition={{
                                                        type: 'spring',
                                                        bounce: 0.2,
                                                        duration: 0.6,
                                ...
                                <AnimatePresence mode="wait">
                                        ...
                                        >
                                            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                                                {[
                                                    {
                                                        l: 'Submissions',
                                                        v: detailTugas.submission_count,
                                                        c: 'from-blue-500 to-indigo-500',
                                                    },
                                                    {
                                                        l: 'Pending',
                                                        v: detailTugas.pending_review,
                                                        c: 'from-amber-500 to-orange-500',
                                                    },
                                                    {
                                                        l: 'Dinilai',
                                                        v: detailTugas.graded_count,
                                                        c: 'from-emerald-500 to-green-500',
                                                    },
                                                    {
                                                        l: 'Terlambat',
                                                        v: detailTugas.late_submissions,
                                                        c: 'from-red-500 to-rose-500',
                                                    },
                                                ].map((s) => (
                                                    <div
                                                        key={s.l}
                                                        className="rounded-xl border border-slate-200/50 bg-white/60 p-3 dark:border-slate-700/50 dark:bg-neutral-800/40"
                                                    >
                                                        ...
                                                        <div
                                                            className={cn(
                                                                'mt-2 h-1 w-12 rounded-full bg-gradient-to-r',
                                                                s.c,
                                                            )}
                                                        />
                                                    </div>
                                            ...
                                            <div className="space-y-3 rounded-xl border border-slate-200/50 bg-white/60 p-4 dark:border-slate-700/50 dark:bg-neutral-800/40">
                                            ...
                                            <div className="flex flex-wrap gap-3">
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() =>
                                                        router.visit(
                                                            `/dosen/tugas/${detailTugas.id}/grading`,
                                                        )
                                                    }
                                                >
                                    ...
                                    {detailTab === 'submissions' && (
                                        <motion.div
                                            ...
                                            className="space-y-4"
                                        >
                                            <div className="rounded-xl border border-slate-200/50 bg-white/60 p-4 dark:border-slate-700/50 dark:bg-neutral-800/40">
                                                ...
                                                <div className="h-3 w-full rounded-full bg-slate-200 dark:bg-slate-700">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${detailTugas.completion_rate}%`,
                                                        }}
                                                    />
                                                </div>
                                            </div>
                                            <div className="grid grid-cols-3 gap-3">
                                                <div className="rounded-xl border bg-emerald-50 p-3 text-center dark:bg-emerald-900/20">
                                                    <p className="text-xl font-bold text-emerald-700 dark:text-emerald-300">
                                                        {detailTugas.submission_count -
                                                            detailTugas.late_submissions}
                                                    </p>
                                                    <p className="text-xs text-emerald-600">
                                                        Tepat Waktu
                                                    </p>
                                                </div>
                                                <div className="rounded-xl border bg-red-50 p-3 text-center dark:bg-red-900/20">
                                                ...
                                                <div className="rounded-xl border bg-slate-50 p-3 text-center dark:bg-slate-800/40">
                                            ...
                                            <div className="rounded-xl border p-4">
                                                ...
                                                <div className="h-2.5 w-full rounded-full bg-slate-200 dark:bg-slate-700">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${detailTugas.submission_count > 0 ? (detailTugas.graded_count / detailTugas.submission_count) * 100 : 0}%`,
                                                        }}
                                                    />
                                                </div>
                                            </div>
                                    ...
                                    {detailTab === 'analytics' && (
                                        ...
                                        >
                                            ...
                                            {detailTugas.days_until_deadline >
                                                0 &&
                                                !detailTugas.is_overdue && (
                                                    <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:bg-amber-900/20">
            ...
            <AnimatePresence>
                {selectedTugas.length > 0 && (
                    <motion.div
                        ...
                        className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-2xl border border-white/20 bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 shadow-2xl backdrop-blur-xl"
                    >
                        <div className="flex items-center gap-4 text-white">
                            <span className="text-sm font-medium">
                                {selectedTugas.length} tugas dipilih
                            </span>
