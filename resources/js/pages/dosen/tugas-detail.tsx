...
import { ArrowLeft, BookOpen, Calendar, MessageSquare, Pin, Send, Sparkles, Zap } from 'lucide-react';
...
export default function DosenTugasDetail({ tugas, diskusi }: Props) {
    ...
    const sendMessage = () => {
        ...
        router.post(`/dosen/tugas/${tugas.id}/message`, { pesan: message, visibility }, {
    ...
    return (
        <DosenLayout>
            ...
            <div className="p-6 space-y-6">
                <Button variant="ghost" onClick={() => router.visit('/dosen/tugas')} className={`mb-4 transition-all duration-500 hover:translate-x-1 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
                ...
                <div className="grid grid-cols-3 gap-6">
                    <div className={`col-span-2 space-y-6 transition-all duration-700 ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
                        <div className="rounded-2xl border bg-card p-6 shadow-lg hover:shadow-xl transition-shadow">
                            ...
                            <h1 className="text-2xl font-bold text-indigo-700 dark:text-indigo-400">{tugas.judul}</h1>
                            ...
                            {tugas.instruksi && (
                                <div className="mt-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-xl">
                                    <h3 className="font-semibold mb-2 flex items-center gap-2"><Zap className="h-4 w-4 text-indigo-500" /> Instruksi</h3>
                    ...
                    <div className={`space-y-4 transition-all duration-700 ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`} style={{ transitionDelay: '200ms' }}>
                        <div className="rounded-2xl border bg-card p-5 shadow-lg">
                            ...
                            <div className="space-y-4 text-sm">
                                <div className="flex justify-between items-center p-2 bg-muted/30 rounded-lg"><span className="text-muted-foreground">Jenis</span><Badge className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white capitalize">{tugas.jenis}</Badge></div>
                ...
                <div className={`rounded-2xl border bg-card shadow-lg transition-all duration-700 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '400ms' }}>
                    <div className="p-4 border-b bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-t-2xl">
                        <h2 className="font-semibold flex items-center gap-2 text-lg">
                            <MessageSquare className="h-5 w-5 text-indigo-500" /> Diskusi ({diskusi.length})
                        </h2>
                    </div>
                    <div className="p-4 space-y-4 max-h-[400px] overflow-y-auto">
                        {diskusi.length === 0 ? (
                            ...
                            diskusi.map((d, i) => (
                                <div key={d.id} className={`flex gap-3 p-3 rounded-xl transition-all duration-300 hover:bg-muted/30 ${d.is_pinned ? 'bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20 border border-amber-200' : ''}`}>
                    ...
                    <div className="p-4 border-t bg-muted/20">
                        ...
                        <div className="flex gap-2">
                            <Textarea placeholder="Tulis pesan..." value={message} onChange={(e) => setMessage(e.target.value)} rows={2} className="flex-1" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} />
                            <Button onClick={sendMessage} className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 shadow-lg transition-all duration-300 hover:scale-105">
