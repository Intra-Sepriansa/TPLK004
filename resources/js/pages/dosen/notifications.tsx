...
import {
    ...
    ExternalLink, X, Trash2, Filter, Search, ShieldAlert, Plus, Send, Inbox, RefreshCw
} from 'lucide-react';
...
interface Notification {
    ...
    title: string;
    message: string;
    type: 'reminder' | 'announcement' | 'alert' | 'achievement' | 'warning' | 'info';
...
interface Props {
    ...
    notifications: { data: Notification[]; current_page: number; last_page: number; total: number; };
    sentNotifications: { data: Notification[]; current_page: number; last_page: number; total: number; };
...
const getTypeIcon = (type: string) => {
    switch (type) {
        ...
        case 'warning': return <ShieldAlert className="h-5 w-5" />;
...
const getTypeColor = (type: string) => {
    switch (type) {
        case 'reminder': return 'text-blue-700 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400';
        case 'announcement': return 'text-purple-700 bg-purple-100 dark:bg-purple-900/30 dark:text-purple-400';
        case 'alert': return 'text-red-700 bg-red-100 dark:bg-red-900/30 dark:text-red-400';
        case 'achievement': return 'text-amber-700 bg-amber-100 dark:bg-amber-900/30 dark:text-amber-400';
        case 'warning': return 'text-orange-700 bg-orange-100 dark:bg-orange-900/30 dark:text-orange-400';
        default: return 'text-neutral-700 bg-neutral-100 dark:bg-neutral-900/30 dark:text-neutral-400';
    }
};

const getPriorityBadge = (priority: string) => {
    switch (priority) {
        case 'urgent': return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">Urgent</span>;
        case 'high': return <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">Penting</span>;
        default: return null;
    }
};
...
export default function Notifications({ dosen, notifications, sentNotifications, stats }: Props) {
    ...
    const activeNotifications = activeTab === 'inbox' ? notifications : sentNotifications;
    ...
    const filteredNotifications = activeNotifications.data.filter(notif => {
        ...
        const matchesType = filterType === 'all' || notif.type === filterType || (filterType === 'urgent' && notif.priority === 'urgent');
        const matchesStatus = filterStatus === 'all' || (filterStatus === 'unread' && !notif.read_at) || (filterStatus === 'read' && notif.read_at);
    ...
    return (
        <DosenLayout dosen={dosen}>
            ...
            <motion.div initial="hidden" animate="visible" variants={containerVariants} className="p-6 space-y-6">
                ...
                {/* TABS NAVIGATION */}
                <motion.div variants={itemVariants} className="flex p-1 gap-1 bg-neutral-100/50 dark:bg-neutral-900/50 rounded-2xl backdrop-blur-md w-fit border border-white/10">
                    <motion.button layout onClick={() => setActiveTab('inbox')} className={`relative px-6 py-2.5 text-sm font-bold rounded-xl transition-all ${activeTab === 'inbox' ? 'text-indigo-700 dark:text-indigo-300 shadow-sm' : 'text-neutral-500 hover:text-neutral-700 dark:text-neutral-400 dark:hover:text-neutral-200'}`}>
                        {activeTab === 'inbox' && <motion.div layoutId="activeTab" className="absolute inset-0 bg-white dark:bg-neutral-800 rounded-xl shadow-sm" transition={{ type: 'spring', bounce: 0.2, duration: 0.6 }} />}
                        <span className="relative z-10 flex items-center gap-2">
                            <Inbox className="h-4 w-4" /> Inbox
                            {stats.unread > 0 && <span className="px-2 py-0.5 rounded-full bg-red-500 text-white text-xs font-bold">{stats.unread}</span>}
                        </span>
                    </motion.button>
                    <motion.button layout onClick={() => setActiveTab('sent')} className={`relative px-6 py-2.5 text-sm font-bold rounded-xl transition-all ${activeTab === 'sent' ? 'text-indigo-700 dark:text-indigo-300 shadow-sm' : 'text-neutral-500 hover:text-neutral-700 dark:text-neutral-400 dark:hover:text-neutral-200'}`}>
                        {activeTab === 'sent' && <motion.div layoutId="activeTab" className="absolute inset-0 bg-white dark:bg-neutral-800 rounded-xl shadow-sm" transition={{ type: 'spring', bounce: 0.2, duration: 0.6 }} />}
                        <span className="relative z-10 flex items-center gap-2">
                ...
                <motion.div variants={itemVariants} className="rounded-3xl border border-white/20 bg-white/50 p-6 shadow-lg backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/50">
                    <div className="flex items-center gap-3 mb-5">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-neutral-500">Temukan notifikasi dengan mudah</p>
                        </div>
                    </div>
                    <div className="grid gap-4 md:grid-cols-4">
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">Cari Notifikasi</label>
                            <div className="relative">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-neutral-400" />
                                <Input type="text" placeholder="Cari judul atau pesan..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 border-2 focus:ring-4 focus:ring-blue-500/20" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">Tipe</label>
                    ...
                    <div className="flex flex-wrap gap-2 pt-4 border-t border-neutral-200/50 dark:border-neutral-800 mt-4">
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setFilterStatus('unread')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filterStatus === 'unread' ? 'bg-orange-500 text-white' : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400'}`}>
                            <AlertTriangle className="h-4 w-4" /> Belum Dibaca
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setFilterType('urgent')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filterType === 'urgent' ? 'bg-red-500 text-white' : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400'}`}>
                            <AlertTriangle className="h-4 w-4" /> Urgent
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleResetFilters} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-neutral-100 dark:bg-neutral-800 text-sm font-medium text-neutral-600 dark:text-neutral-400 transition-colors hover:bg-neutral-200 dark:hover:bg-neutral-700">
                            <X className="h-4 w-4" /> Reset
                        </motion.button>
                    </div>
                    <div className="flex items-center justify-between pt-2 mt-2 border-t border-neutral-200/50 dark:border-neutral-800">
                        <span className="text-sm text-neutral-500">Menampilkan {filteredNotifications.length} dari {activeNotifications.data.length} notifikasi</span>
                    </div>
                </motion.div>
                    ...
                    <div className="divide-y divide-neutral-200 dark:divide-neutral-800">
                        <AnimatePresence mode="popLayout">
                            {filteredNotifications.map((notif, idx) => (
                                <motion.div
                                    ...
                                    whileHover={{ x: 5, backgroundColor: !notif.read_at && activeTab === 'inbox' ? 'rgba(99, 102, 241, 0.08)' : 'rgba(59, 130, 246, 0.05)', scale: 1.01 }}
                                    className={`p-5 transition-all cursor-pointer relative overflow-hidden ${!notif.read_at && activeTab === 'inbox' ? 'bg-gradient-to-r from-blue-50/50 to-indigo-50/30 dark:from-blue-900/10 dark:to-indigo-900/10' : ''}`}
                                    onClick={() => handleOpenDetail(notif)}
                                >
                                    {!notif.read_at && activeTab === 'inbox' && (
                                        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.5, delay: idx * 0.05 }} className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-indigo-500 to-purple-600" />
                                    )}
                                    <div className="flex items-start gap-4">
                                        <motion.div whileHover={{ scale: 1.15, rotate: 10 }} whileTap={{ scale: 0.95 }} className={`flex h-12 w-12 items-center justify-center rounded-xl shrink-0 shadow-lg ${getTypeColor(notif.type)}`}>
                                            {getTypeIcon(notif.type)}
                                        </motion.div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 flex-wrap mb-1">
                                                <span className="font-semibold text-neutral-900 dark:text-white">{notif.title}</span>
                                                {getPriorityBadge(notif.priority)}
                                                {!notif.read_at && activeTab === 'inbox' && (
                                                    <motion.span animate={{ scale: [1, 1.3, 1], opacity: [1, 0.7, 1] }} transition={{ duration: 2, repeat: Infinity }} className="flex h-2 w-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 shadow-lg shadow-blue-500/50" />
                                                )}
                                            </div>
                                            <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1 line-clamp-2 leading-relaxed">{notif.message}</p>
                                            <div className="flex items-center gap-4 mt-2">
                                                <span className="text-xs text-neutral-500 flex items-center gap-1"><Clock className="h-3 w-3" /> {formatTime(notif.created_at)}</span>
                                                <motion.span whileHover={{ x: 5 }} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium flex items-center gap-1">
                                                    Baca selengkapnya <motion.span animate={{ x: [0, 3, 0] }} transition={{ duration: 1.5, repeat: Infinity }}>→</motion.span>
                                                </motion.span>
                                            </div>
                                        </div>
                                        <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                                            <Button size="icon" variant="ghost" className="text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 shrink-0" onClick={(e) => { e.stopPropagation(); openDeleteDialog(notif.id); }}>
                        ...
                        {filteredNotifications.length === 0 && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-16 text-center">
                                <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} className="mb-6">
                                ...
                                <h3 className="text-lg font-semibold text-neutral-900 dark:text-white mb-2">Tidak ada notifikasi</h3>
                                <p className="text-sm text-neutral-500">Notifikasi baru akan muncul di sini</p>
                            </motion.div>
                        )}
                    </div>
                </motion.div>
                ...
                {activeNotifications.last_page > 1 && (
                    <motion.div variants={itemVariants} className="flex justify-center mt-6 gap-2">
                        {Array.from({ length: activeNotifications.last_page }, (_, i) => i + 1).map(page => (
                            <motion.button
                                key={page}
                ...
                <AnimatePresence>
                    {detailModal.open && detailModal.notification && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setDetailModal({ open: false, notification: null })}>
                            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: 'spring', stiffness: 400, damping: 17 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-2xl rounded-3xl bg-white/90 p-8 shadow-2xl dark:bg-neutral-900/90 backdrop-blur-xl border border-white/20 dark:border-neutral-800 max-h-[90vh] overflow-y-auto">
                                <div className="flex items-start justify-between mb-6">
                                    <div className="flex items-center gap-4">
                                        <motion.div initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: 'spring', stiffness: 400, damping: 17 }} className={`flex h-14 w-14 items-center justify-center rounded-2xl shadow-lg ${getTypeColor(detailModal.notification.type)}`}>
                                            {getTypeIcon(detailModal.notification.type)}
                                        </motion.div>
                                        <div>
                                            <div className="flex gap-2">
                                                <motion.span initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className={`text-xs px-2.5 py-0.5 rounded-full font-medium ${getTypeColor(detailModal.notification.type)}`}>
                                                    {detailModal.notification.type.toUpperCase()}
                                                </motion.span>
                                                {getPriorityBadge(detailModal.notification.priority)}
                                            </div>
                                            <motion.h3 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="text-xl font-bold text-neutral-900 dark:text-white mt-1">
                                                {detailModal.notification.title}
                                            </motion.h3>
                                        </div>
                                    </div>
                                    <motion.button whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }} onClick={() => setDetailModal({ open: false, notification: null })} className="text-neutral-400 hover:text-neutral-700 dark:hover:text-white bg-neutral-100 dark:bg-neutral-800 p-2 rounded-full">
                                        <X className="h-5 w-5" />
                                    </motion.button>
                                </div>

                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-6 flex flex-wrap items-center gap-3 text-sm text-neutral-600 dark:text-neutral-400">
                                    <div className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> {formatFullDate(detailModal.notification.created_at)}</div>
                                </motion.div>

                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="prose prose-slate dark:prose-invert max-w-none">
                                    <div className="p-6 bg-white dark:bg-black rounded-2xl whitespace-pre-wrap text-neutral-700 dark:text-neutral-300 leading-relaxed border border-neutral-100 dark:border-neutral-800 shadow-inner">
                                        {detailModal.notification.message}
                                    </div>
                                </motion.div>

                                {detailModal.notification.action_url && (
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-6 flex justify-end">
                                        <Button asChild className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white shadow-lg shadow-indigo-500/30 rounded-xl px-6 py-6 border-0 group">
                                            <a href={detailModal.notification.action_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 font-bold text-md">
                                                {detailModal.notification.action_label || 'Buka Tautan'}
