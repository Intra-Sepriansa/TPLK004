            ...
            <motion.div initial="hidden" animate="visible" variants={containerVariants} className="p-6 space-y-6">
                    ...
                    <div className="relative">
                        <div className="flex flex-wrap items-center justify-between gap-6">
                            <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 text-center sm:text-left w-full lg:w-auto">
                                ...
                                >
                                    <img src={PenilaianIcon} alt="Penilaian" className="absolute inset-0 h-full w-full object-contain drop-shadow-[0_15px_25px_rgba(0,0,0,0.6)]" />
                                </motion.div>
                ...
                {grades && (<>
                    ...
                    <motion.div variants={containerVariants} className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                        {[
                            { key: 'students', label: 'Total Mahasiswa', value: stats.total_students, icon: MahasiswaIcon, hoverShadow: 'hover:shadow-violet-500/10', gradBg: 'from-violet-500/5 to-purple-500/5 dark:from-violet-500/10 dark:to-purple-500/10' },
                            { key: 'average', label: 'Rata-rata Kehadiran', value: stats.average_attendance_rate, suffix: '%', icon: RataRataIcon, hoverShadow: 'hover:shadow-emerald-500/10', gradBg: 'from-emerald-500/5 to-teal-500/5 dark:from-emerald-500/10 dark:to-teal-500/10' },
                            { key: 'sessions', label: 'Total Pertemuan', value: stats.total_sessions, icon: SesiIcon, hoverShadow: 'hover:shadow-blue-500/10', gradBg: 'from-blue-500/5 to-cyan-500/5 dark:from-blue-500/10 dark:to-cyan-500/10' },
                            { key: 'risk', label: 'Tidak Bisa UAS', value: stats.students_at_risk, icon: BeresikoIcon, hoverShadow: 'hover:shadow-red-500/10', gradBg: 'from-red-500/5 to-rose-500/5 dark:from-red-500/10 dark:to-rose-500/10' },
                        ].map(card => (
                            <motion.div key={card.key} variants={cardVariants} whileHover="hover" onHoverStart={() => setHoveredCard(card.key)} onHoverEnd={() => setHoveredCard(null)}
                                ...
                                <motion.div animate={{ scale: hoveredCard === card.key ? 1.5 : 1, opacity: hoveredCard === card.key ? 0.4 : 0.2 }} className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-white blur-3xl transition-all duration-500" />
                                            ...
                                            <motion.tr key={g.mahasiswa_id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: idx * 0.02 }} whileHover={{ x: 5, backgroundColor: 'rgba(99,102,241,0.05)' }} className="transition-colors">
                                                ...
                                                <td className="px-4 py-4 text-center">
                                                    <div className="flex items-center justify-center gap-1.5">
                                                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setDetailModal({ open: true, student: g })} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-600 text-white text-xs font-medium shadow-lg shadow-blue-500/30">
                                                            <Eye className="h-3 w-3" /> Quick
                                                        </motion.button>
                                                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => router.visit(`/dosen/grading/detail/${g.mahasiswa_id}`)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xs font-medium shadow-lg shadow-indigo-500/30">
