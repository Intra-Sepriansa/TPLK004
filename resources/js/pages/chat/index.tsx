...
import { ContactInfoModal, MessageInfoModal } from '@/components/ui/info-modal';
import { Input } from '@/components/ui/input';
import { ToastProvider, useToast } from '@/components/ui/toast-notification';
import { apiDelete, apiGet, apiPost, apiPut, isCsrfError } from '@/lib/api';
...
function ChatContent({
        ...
        try {
            ...
            if (response.ok) {
                ...
                showError(
                    ...
                    errorData.error || errorData.message || 'Terjadi kesalahan',
                );
            }
        } catch (error) {
    ...
    const handleTyping = async (isTyping: boolean) => {
        ...
        try {
            await apiPost(
                `/api/chat/conversations/${activeConversation.id}/typing`,
                { is_typing: isTyping },
            );
        } catch (error) {}
    };
    ...
    const handleEdit = async (message: Message, newContent: string) => {
        try {
            const response = await apiPut(`/api/chat/messages/${message.id}`, {
                content: newContent,
            });
            if (response.ok) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await apiDelete(
                `/api/chat/messages/${deleteDialog.message.id}`,
            );
            if (response.ok) {
                router.reload({ only: ['activeConversation'] });
            }
    ...
    const handleForward = async (message: Message) => {
        ...
        try {
            const response = await apiPost(
                `/api/chat/messages/${message.id}/forward`,
                { conversation_ids: [parseInt(targetConvId)] },
            );
            if (response.ok) {
                const data = await response.json();
                showSuccess(
                    'Pesan diteruskan',
                    data.message || 'Berhasil meneruskan pesan',
                    'forward',
                );
            } else {
    ...
    const handleReact = async (message: Message, emoji: string) => {
        try {
            await apiPost(`/api/chat/messages/${message.id}/reactions`, {
                emoji,
            });
            router.reload({ only: ['activeConversation'] });
        } catch (error) {}
    ...
    const searchContacts = useCallback(async (query: string) => {
        ...
        try {
            const response = await apiGet(
                `/chat/contacts/search?q=${encodeURIComponent(query)}`,
            );
            const data = await response.json();
    ...
    return (
        <>
            ...
            <div className="flex h-screen bg-[#111b21]">
                ...
                <div className="absolute top-0 right-0 left-0 z-20 flex items-center gap-3 border-b border-[#222d34] bg-[#202c33] px-4 py-3">
                    <Button
                        variant="ghost"
                        size="icon"
                ...
                <div
                    className={cn(
                        'w-full max-w-[500px] min-w-[300px] border-r border-[#222d34] pt-16 md:w-[30%] lg:w-[25%]',
                        'md:block',
                ...
                >
                    <ConversationList
                        ...
                        onArchive={async (conv) => {
                            try {
                                const response = await apiPost(
                                    `/api/chat/conversations/${conv.id}/archive`,
                                );
                                if (response.ok) {
                                    const data = await response.json();
                                    showSuccess(
                        ...
                        onPin={async (conv) => {
                            try {
                                const response = await apiPost(
                                    `/api/chat/conversations/${conv.id}/pin`,
                                );
                                if (response.ok) {
                                    const data = await response.json();
                        ...
                        onMute={async (conv) => {
                            try {
                                const response = await apiPost(
                                    `/api/chat/conversations/${conv.id}/mute`,
                                );
                                if (response.ok) {
                                    const data = await response.json();
                                    showSuccess(
                                ...
                                showError(
                                    'Gagal',
                                    'Tidak dapat membisukan notifikasi',
                                );
                            }
                        }}
                        onContactInfo={async (conv) => {
                            try {
                                const response = await apiGet(
                                    `/api/chat/conversations/${conv.id}/info`,
                                );
                                if (response.ok) {
                                    const data = await response.json();
                                ...
                                showError(
                                    'Gagal',
                                    'Tidak dapat memuat info kontak',
                                );
                            }
                        }}
                    />
                    ...
                    {activeConversation ? (
                        <ChatWindow
                            ...
                            onStar={async (message) => {
                                try {
                                    const response = await apiPost(
                                        `/api/chat/messages/${message.id}/star`,
                                    );
                                    if (response.ok) {
                                        const data = await response.json();
                                        showSuccess(
                            ...
                            onPin={async (message) => {
                                try {
                                    const response = await apiPost(
                                        `/api/chat/messages/${message.id}/pin`,
                                    );
                                    if (response.ok) {
                                        const data = await response.json();
                                        showSuccess(
                            ...
                            onInfo={async (message) => {
                                try {
                                    const response = await apiGet(
                                        `/api/chat/messages/${message.id}/info`,
                                    );
                                    if (response.ok) {
                                        const data = await response.json();
                        ...
                        <div className="flex flex-1 flex-col items-center justify-center bg-[#222e35]">
                            <div className="max-w-md px-8 text-center">
                                {/* Icon Komunikasi Kelas */}
                                <div className="mx-auto mb-8 flex h-32 w-32 items-center justify-center rounded-full bg-[#00a884]/10">
                                    <svg
                                        viewBox="0 0 24 24"
                                        className="h-16 w-16 text-[#00a884]"
                                        fill="none"
                                        stroke="currentColor"
                                        strokeWidth="1.5"
                                    >
                                ...
                                <h1 className="mb-3 text-[28px] font-medium text-[#e9edef]">
                                    Komunikasi Kelas
                                </h1>
                                <p className="mb-6 text-sm leading-relaxed text-[#8696a0]">
                                    Mulai percakapan dengan dosen atau mahasiswa
                                    lain.
                                    <br />
                                    Diskusi tugas, tanya jawab, dan koordinasi
                                    kelas jadi lebih mudah.
                                </p>
                                <div className="flex flex-col gap-3">
                                    <button
                                        onClick={() => setShowNewChat(true)}
                                        className="flex items-center justify-center gap-2 rounded-lg bg-[#00a884] px-6 py-3 font-medium text-white transition-colors hover:bg-[#06cf9c]"
                                    >
            ...
            <Dialog open={showNewChat} onOpenChange={setShowNewChat}>
                <DialogContent className="border-[#222d34] bg-[#111b21] text-[#e9edef] sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-3 text-[#e9edef]">
                            <Button
                                ...
                                className="text-[#aebac1] hover:bg-[#202c33]"
                            >
                                <ArrowLeft className="h-5 w-5" />
                            </Button>
                            Chat baru
                    ...
                    <div className="space-y-2">
                        <div className="relative px-3">
                            <Search className="absolute top-1/2 left-6 h-4 w-4 -translate-y-1/2 text-[#8696a0]" />
                            <Input
                                placeholder="Cari nama atau NIM"
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="h-9 rounded-lg border-0 bg-[#202c33] pl-10 text-[#e9edef] placeholder:text-[#8696a0]"
                                autoFocus
                            />
                        </div>
                                ...
                                <div>
                                    {searchResults.map((contact) => (
                                        <button
                                            ...
                                            className="flex w-full items-center gap-3 px-3 py-2 transition-colors hover:bg-[#202c33]"
                                        >
                                            {contact.avatar ? (
                                                <img
                                                    src={contact.avatar}
                                                ...
                                                <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-[#6b7c85]">
                                                    <svg
                                                        viewBox="0 0 212 212"
                                                        className="h-12 w-12"
                                                    >
                                            ...
                                            <div className="flex-1 border-b border-[#222d34] py-2 text-left">
            ...
            <ConfirmDialog
                ...
                onOpenChange={(open) =>
                    setDeleteDialog({
                        open,
                        message: open ? deleteDialog.message : null,
                    })
                }
