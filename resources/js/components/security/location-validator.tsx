...
export function LocationValidator({
    ...
    const [status, setStatus] = useState<
        'idle' | 'collecting' | 'validating' | 'success' | 'failed'
    >('idle');
    const [failureReason, setFailureReason] = useState<string | null>(null);

    const distance =
        latitude && longitude ? calculateDistance(targetLat, targetLng) : null;
    const isInRange =
        latitude && longitude
    ...
    // Collect samples
    useEffect(() => {
        if (status !== 'collecting' || !latitude || !longitude || !timestamp)
            return;
    ...
    const validateSamples = () => {
        ...
        if (!allInRange) {
            setStatus('failed');
            setFailureReason(
                'Beberapa sampel lokasi berada di luar area yang diizinkan',
            );
        ...
        if (spread > maxSpreadMeters) {
            setStatus('failed');
            setFailureReason(
                'Lokasi tidak konsisten. Kemungkinan GPS spoofing terdeteksi.',
            );
        ...
        const avgAccuracy =
            samples.reduce((sum, s) => sum + s.accuracy, 0) / samples.length;
        if (avgAccuracy > 100) {
            setStatus('failed');
            setFailureReason(
                'Akurasi GPS terlalu rendah. Coba di area terbuka.',
            );
                    ...
                    <div className="flex-1">
                        ...
                        {status === 'collecting' && (
                            <div className="mt-3">
                                ...
                                <Progress
                                    value={
                                        (samples.length / requiredSamples) * 100
                                    }
                                    className="h-2"
                                />
                            </div>
